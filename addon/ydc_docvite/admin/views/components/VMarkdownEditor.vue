<template>
  <div class="vmdContainer">
    ...
    <el-card shadow="never">
      ...
      <el-button type="danger" @click="doReset">{{ trans("reset") }}</el-button>
    </el-card>
    <upload-attachment
      type="image"
      ref="imageRef"
...
export default {
  ...
  watch: {
    content(newVal) {
      if (newVal !== "" && this.editorInit && this.vdtr?.getValue() === "") {
        console.debug("watch load content");
        this.vdtr?.setValue(newVal);
      }
    },
  },
  components: {},
  ...
  methods: {
    ...
    async doConfirm() {
      this.$emit("on-confirm", {
        ...
        attachs: this.attachIds,
      });
    },
    doReset() {
      this.vdtr.setValue(this.content, true);
    },
    imageSelected(data) {
