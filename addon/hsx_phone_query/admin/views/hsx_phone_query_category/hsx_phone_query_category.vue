<template>
    <div class="main-container">
        <el-card class="box-card !border-none" shadow="never">
            ...
            <el-card class="box-card !border-none my-[10px] table-search-wrap" shadow="never">
                <el-form :inline="true" :model="hsxPhoneQueryCategoryTable.searchParam" ref="searchFormRef">
                    ...
                    <el-form-item :label="t('typeId')" prop="type_id">
                        <el-select class="w-[280px]" v-model="hsxPhoneQueryCategoryTable.searchParam.type_id" clearable :placeholder="t('typeIdPlaceholder')">
                            <el-option label="全部" value=""></el-option>
                            <el-option
                                v-for="(item, index) in type_idList"
                                :key="index"
                                :label="item.name"
                                :value="item.value"
                            />
                    ...
                    <el-form-item :label="t('name')" prop="name">
                        <el-input v-model="hsxPhoneQueryCategoryTable.searchParam.name" :placeholder="t('namePlaceholder')" />
                    </el-form-item>
                    <el-form-item :label="t('price')" prop="price">
                        <el-input v-model="hsxPhoneQueryCategoryTable.searchParam.price" :placeholder="t('pricePlaceholder')" />
                    </el-form-item>
                    <el-form-item>
            ...
            <div class="mt-[10px]">
                <el-table :data="hsxPhoneQueryCategoryTable.data" size="large" v-loading="hsxPhoneQueryCategoryTable.loading">
                    <template #empty>
                    ...
                    <el-table-column :label="t('operation')" fixed="right" min-width="120">
                       <template #default="{ row }">
                           ...
                           <el-button type="primary" link @click="deleteEvent(row.site_id)">{{ t('delete') }}</el-button>
                       </template>
                    </el-table-column>

                </el-table>
                <div class="mt-[16px] flex justify-end">
                    <el-pagination v-model:current-page="hsxPhoneQueryCategoryTable.page" v-model:page-size="hsxPhoneQueryCategoryTable.limit"
                        layout="total, sizes, prev, pager, next, jumper" :total="hsxPhoneQueryCategoryTable.total"
                        @size-change="loadHsxPhoneQueryCategoryList()" @current-change="loadHsxPhoneQueryCategoryList" />
                </div>
            </div>
...
// 字典数据
    ...
    type_idList.value = await (await useDictionary('分类')).data.dictionary
    }
    type_idDictList();
