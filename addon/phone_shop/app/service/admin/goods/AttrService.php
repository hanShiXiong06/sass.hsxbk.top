...
class AttrService extends BaseAdminService
{
    ...
    public function getPage(array $where = [], $field = 'attr_id,site_id,attr_name,sort')
    {
        ...
        $search_model = $this->model->where([ [ 'site_id', 'in', "{$this->site_id},100005" ] ])->withSearch([ "attr_id", "attr_name" ], $where)->field($field)->order($order);
    ...
    public function getList(array $where = [], $field = 'attr_id,site_id,attr_name,sort')
    {
        ...
        return $this->model->where([ [ 'site_id', 'in', "{$this->site_id},100005" ] ])->withSearch([ "attr_id", "attr_name" ], $where)->field($field)->order($order)->select()->toArray();
    }
