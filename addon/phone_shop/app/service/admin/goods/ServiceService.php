...
class ServiceService extends BaseAdminService
{
    ...
    public function getPage(array $where = [])
    {
        ...
        $search_model = $this->model->where([ [ 'site_id', 'in', "{$this->site_id},100005" ] ])->withSearch([ "service_name" ], $where)->field($field)->order($order);
    ...
    {
        $order = "create_time desc";
        return $this->model->where([ [ 'site_id', 'in', "{$this->site_id},100005" ] ])->withSearch([ "service_name" ], $where)->field($field)->order($order)->select()->toArray();
    }
