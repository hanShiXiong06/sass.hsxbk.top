...
class CategoryService extends BaseAdminService
{
    ...
    public function getPage(array $where = [])
    {
        ...
        // 
        $search_model = $this->model->where([[ 'site_id', 'in', "{$this->site_id},100005" ]   ])->withSearch([ "category_name" ], $where)->field($field)->order($order);
    ...
    public function getList(array $where = [], $field = 'category_id,category_name,image,level,pid,category_full_name,is_show,sort')
    {
        ...
        return $this->model->where([ [ 'site_id', 'in', "{$this->site_id},100005" ]  ])->withSearch([ "category_name", 'level' ], $where)->field($field)->order($order)->select()->toArray();
    }
    ...
    {
        return ( new CoreGoodsCategoryService() )->getTree([ [ 'site_id', 'in', "{$this->site_id},100005" ]  ]);
    }
