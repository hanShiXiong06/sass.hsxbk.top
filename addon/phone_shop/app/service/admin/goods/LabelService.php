...
class LabelService extends BaseAdminService
{
    ...
    {
        $field = 'label_id, label_name, group_id, style_type, color_json, icon, status,  site_id,memo, sort, create_time, update_time';
        $order = 'label_id desc';
        if (!empty($where[ 'order' ])) {
            $order = $where[ 'order' ] . ' ' . $where[ 'sort' ];
        }
        if($this->site_id !== 0 ){
            $sites =  (new Site())-> field('label_status')->where([['site_id','=', $this->site_id]]) ->findOrEmpty()->toArray();
            
        }
        
        $site_id = empty($sites['label_status'] ) ? $this->site_id : $this->site_id.",0";
       
        $search_model = $this->model->where([ [ 'site_id',  'in', $site_id ] ])->withSearch([ "label_name", 'group_id' ], $where)->field($field)
            ->with([
     ...
     * @return array
     */
    public function getList(array $where = [], $field = 'label_id, label_name, group_id,  site_id,style_type, color_json, icon, status, memo, sort, create_time, update_time')
    {
        ...
        $site_id = empty($sites['label_status'] ) ? $this->site_id : $this->site_id.",0";
        return $this->model->where([ [ 'site_id', 'in', $site_id ] ])->withSearch([ "label_name" ], $where)->field($field)
            ->with([
                'group' => function($query) {
                    $query->field('group_id, group_name');
                },
    ...
    {
        $field = 'label_id, label_name, group_id, style_type, color_json, icon, status, memo, sort, site_id, create_time, update_time';
        if($this->site_id !== 0 ){
            $sites =  (new Site())-> field('label_status')->where([['site_id','=', $this->site_id]]) ->findOrEmpty()->toArray();
