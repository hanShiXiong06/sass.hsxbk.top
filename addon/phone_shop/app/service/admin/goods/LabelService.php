...
class LabelService extends BaseAdminService
{
    ...
    {
        $field = 'label_id,label_name,memo,sort,site_id,create_time,update_time';
        $order = 'label_id desc';
        if (!empty($where[ 'order' ])) {
            $order = $where[ 'order' ] . ' ' . $where[ 'sort' ];
        }

        $search_model = $this->model->where([ [ 'site_id', 'in', "{$this->site_id},100005" ] ])->withSearch([ "label_name" ], $where)->field($field)->order($order);
     ...
     * @return array
     */
    public function getList(array $where = [], $field = 'label_id,site_id,label_name,memo,sort,create_time,update_time')
    {
        $order = 'sort asc';
        return $this->model->where([ [ 'site_id', 'in', "{$this->site_id},100005" ] ])->withSearch([ "label_name" ], $where)->field($field)->order($order)->select()->toArray();
    }
