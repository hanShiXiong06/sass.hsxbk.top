...
use addon\phone_shop\app\model\delivery\ShippingTemplate;
use addon\phone_shop\app\model\delivery\ShippingTemplateItem;
...
class ShippingTemplateService extends BaseAdminService
{
    ...
    public function getPage(array $where)
    {
        ...
        $search_model = ( new ShippingTemplate() )->where([  [ 'site_id', 'in', "{$this->site_id},100005" ], ])->withSearch([ 'template_name' ], $where)->field($field)->order('create_time desc')->append([ 'fee_type_name' ]);
    ...
    public function getList(array $where = [], $field = 'template_id, template_name, fee_type, create_time, no_delivery, is_free_shipping')
    {
        ...
        return ( new ShippingTemplate() )->where([  [ 'site_id', 'in', "{$this->site_id},100005" ], ])->withSearch([ "template_name" ], $where)->field($field)->order($order)->select()->toArray();
    }
    ...
    {
        $field = 'template_id, template_name, fee_type, create_time, no_delivery, is_free_shipping';
        $detail = ( new ShippingTemplate() )->where([ [ 'template_id', '=', $template_id ],  [ 'site_id', 'in', "{$this->site_id},100005" ], ])->field($field)->append([ 'fee_type_name' ])->findOrEmpty()->toArray();
        if (!empty($detail)) {
