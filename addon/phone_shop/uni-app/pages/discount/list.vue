<template>
	<view class="discount bg-[#f6f6f6] min-h-[100vh]" :style="themeColor()">
		...
		<view class="fixed top-0 left-0 w-full z-10 text-[0]">
			...
			<!-- #endif -->
			<u-swiper v-if="bannerList.length" :list="imgList" :indicator="bannerList.length"
				:indicatorStyle="{ 'bottom': '46rpx', }" :autoplay="true" :height="headStyle"
				@click="toRedirect"></u-swiper>
			<image v-if="!bannerListLoading && !bannerList.length"
				:src="img('addon/phone_shop/discount/discount_banner.png')" mode="scaleToFill" class="w-full"
				:style="{ height: headStyle }" :show-menu-by-longpress="true" />
			<view class="relative w-full h-[110rpx] mt-[-40rpx] z-5" v-if="discountList.length">
				<view
					class="bg-[#f24f3d] w-[750rpx] rounded-tl-[24rpx] rounded-tr-[24rpx] h-[96rpx] absolute left-0 bottom-0">
				</view>
				<scroll-view :scroll-x="true" class="h-[110rpx] absolute left-0 bottom-0 z-5">
					<view class="flex items-end h-[100%]" :style="{ 'width': 187.5 * discountList.length + 'rpx' }">
						<view class="w-[187.5rpx] h-[96rpx] relative flex-shrink-0" v-for="(item, index) in discountList"
							@click="navClick(item)">
							<view class="w-full absolute left-0 top-0 z-10 text-[#fff] text-center pt-[10rpx]">
								<view class="text-[28rpx] leading-[39rpx] font-500 px-[10rpx] h-[39rpx] overflow-hidden"
									:class="{ '!text-[#333]': active == item.active_id }">{{ item.active_desc }}</view>
								<view class="flex justify-center w-full">
									<text class="text-[22rpx] leading-[34rpx] font-400 mt-[5rpx]"
		...
		<mescroll-body v-if="discountList.length" ref="mescrollRef" :top="mescrollTop" @init="mescrollInit"
			:down="{ use: false }" @up="getActiveDiscountGoodsListFn">
			<view class="sidebar-marign bg-[#F4F6F8]">
				<block v-for="(item, index) in list" :key="index">
					<view class="bg-[#fff] p-[20rpx] flex rounded-[16rpx]"
						:class="{ 'mb-[20rpx]': index < list.length - 1, 'mb-[30rpx]': index == list.length - 1 }"
						@click="toLink(item)">
						<view class="w-[240rpx] h-[240rpx]">
							<u--image width="240rpx" height="240rpx" radius="16rpx"
								:src="img(item.goods_cover_thumb_mid ? item.goods_cover_thumb_mid : '')"
								model="aspectFill">
								<template #error>
									<image class="rounded-[16rpx] overflow-hidden w-[240rpx] h-[240rpx]"
						...
						<view class="ml-[20rpx] flex-1 flex flex-col justify-between">
							<view>
								...
								<view v-if="item.goods_label_name && item.goods_label_name.length"
									class=" mt-[10rpx] text-[#999999] text-[24rpx] font-400 multi-hidden">
							...
							<view class="w-full h-[80rpx] bg-[#FFECEC] flex justify-between rounded-[40rpx]">
								<view class="mr-[20rpx] pl-[20rpx] flex-1 flex flex-col justify-center">
									<view class="text-[#EF000C] flex items-baseline">
										...
										<text
											class="text-[32rpx] leading-[40rpx] font-500 price-font">{{ parseFloat(item.goodsSku.active_discount_price).toFixed(2).split('.')[0] }}.</text>
										<text
											class="text-[22rpx] leading-[28rpx] font-500 price-font">{{ parseFloat(item.goodsSku.active_discount_price).toFixed(2).split('.')[1] }}</text>
										<view v-if="item.goodsSku.active_discount_rate < 10"
											class="px-[4rpx] border-[1rpx] border-[#EF000C] border-solid  text-[18rpx] ml-[6rpx] rounded-[4rpx] leading-[24rpx] transform translate-y-[-4rpx]">
											{{ item.goodsSku.active_discount_rate }}折</view>
									</view>
									<view class="flex items-center mt-[2rpx]">
										<view
											class="w-[20rpx] h-[20rpx] mr-[4rpx] rounded-[12.5rpx] text-[#fff] bg-[#EF000C] flex items-center justify-center"
											:class="{ '!bg-[#FF8540]': item.activeGoods.active_goods_status != 'active' }">
											<text class="text-[10rpx] iconzhekou iconfont"></text>
										</view>
										<view class="text-[18rpx] font-400 text-[#EF000C] leading-[24rpx]"
											:class="{ '!text-[#FF8540]': item.activeGoods.active_goods_status != 'active' }">
											已省{{ item.goodsSku.active_reduce_money }}元</view>
									</view>
								</view>
								<view
									class="active-button w-[106rpx] h-[78rpx] box-border pl-[25rpx] leading-[78rpx] text-[#fff]"
									:class="{ '!w-[116rpx]': item.activeGoods.active_goods_status != 'active' }"
									:style="{ 'background-image': 'url(' + img(item.activeGoods.active_goods_status != 'active' ? 'addon/phone_shop/discount/no-active.png' : 'addon/phone_shop/discount/active.png') + ')' }">
									<text v-if="item.activeGoods.active_goods_status != 'active'"
										class="text-[26rpx] font-500">{{ item.activeGoods.active_goods_status_name }}</text>
				...
				<view class="flex justify-center w-[100%] items-center "
					:class="{ 'h-[calc(100vh-620rpx)]': discountList.length, 'h-[calc(100vh-550rpx)]': !discountList.length }"
					v-if="!list.length && !loading">
					<mescroll-empty :option="{ tip: '暂无商品，请看看其他商品吧！' }"></mescroll-empty>
				</view>
			</view>
		</mescroll-body>
		<view class="flex flex-col justify-center items-center h-[calc(100vh-550rpx)] fixed left-[30rpx] right-[30rpx]"
			:style="{ 'top': mescrollTop }" v-if="!discountList.length && !loading">
			<u-empty text="暂无活动，请看看其他商品吧！" width="347rpx" height="265rpx"
				:icon="img('static/resource/images/system/empty.png')" />
			<button shape="circle" plain="true"
				class="w-[220rpx] mt-[42rpx] text-[28rpx] h-[66rpx] leading-[62rpx] !text-[#EF000C] !border-[#EF000C] border-[2rpx] rounded-full"
...
const toLink = (item: any) => {
	...
	redirect({ url: '/addon/phone_shop/pages/goods/detail', param: { sku_id: item.goodsSku.sku_id } })
}
