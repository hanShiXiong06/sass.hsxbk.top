<template>
	<u-popup :show="show" @close="show = false" mode="bottom" :round="10" :closeable="true" :safeAreaInsetBottom="true"
		:customStyle="{ maxHeight: '58vh' }">
							...
							<u-form-item label="需要发票" :border-bottom="true">
								<view class="flex">
									<view
										...
										:class="{ 'bg-primary text-white !border-primary': !need }" @click="need = false">
										不需要</view>
									<view
										class="rounded px-[30rpx] py-[10rpx] border-1 !border-[#eee] border-solid text-sm"
										:class="{ 'bg-primary text-white !border-primary': need }" @click="need = true">需要
									</view>
								</view>
							</u-form-item>
						</view>
						<view v-show="need">
								...
								<u-form-item label="发票内容" prop="header_name" :border-bottom="true">
									<view class="flex flex-wrap">
										<block v-for="item in config.invoice_content">
											<view
												class="rounded px-[30rpx] my-[10rpx] py-[10rpx] mr-[20rpx] border-1 !border-[#eee] border-solid text-sm whitespace-normal"
							...
							<view class="mt-[10rpx]">
								<u-form-item label="发票抬头" prop="header_name" :border-bottom="true">
									<u-input v-model.trim="formData.header_name" border="none" clearable
							...
							<view v-if="formData.header_type == 2">
								<view class="mt-[10rpx]">
									<u-form-item label="纳税人识别号" prop="tax_number" :border-bottom="true">
										<u-input v-model.trim="formData.tax_number" border="none" clearable
											placeholder="请输入纳税人识别号" @change="inputChange"></u-input>
									</u-form-item>
								</view>
								<view class="py-[20rpx] flex items-baseline">
									...
									<view class="text-xs text-right flex-1" @click="optionalShow = !optionalShow">
										...
										<text class="text-[30rpx] nc-iconfont text-gray-subtitle ml-[5rpx]"
											:class="optionalShow ? 'nc-icon-shangV6xx-1' : 'nc-icon-xiaV6xx'"></text>
									</view>
								</view>
