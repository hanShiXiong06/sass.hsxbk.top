						...
						<view class="flex mt-[20rpx]">
							<view class="w-[150rpx] h-[150rpx]">
								<u--image class="rounded-[10rpx] overflow-hidden" width="150rpx" height="150rpx"
									:src="img(item.order_goods.goods_image_thumb_small ? item.order_goods.goods_image_thumb_small : '')"
									model="aspectFill">
									<template #error>
										<image class="rounded-[10rpx] overflow-hidden w-[150rpx] h-[150rpx]"
							...
							<view class="ml-[20rpx] flex flex-1 flex-col box-border">
								<view class="flex justify-between items-baseline">
									<view
										class="text-[28rpx] leading-[40rpx] text-[#333] max-w-[322rpx]  truncate font-500">
										{{
											item.order_goods.goods_name }}</view>
									<view class="text-right leading-[42rpx] ml-[10rpx]">
										<text class="text-[20rpx] price-font">￥</text>
										<text
											class="text-[32rpx] font-500 price-font">{{ parseFloat(item.order_goods.price).toFixed(2).split('.')[0]
											}}</text>
										<text
											class="text-[22rpx] font-500 price-font">.{{ parseFloat(item.order_goods.price).toFixed(2).split('.')[1]
											}}</text>
									</view>
								</view>
								<view class="flex justify-between items-center mt-[20rpx]">
									<view
										class="text-[26rpx] truncate font-400 text-[#333] leading-[36rpx] max-w-[369rpx]">
										{{
											item.order_goods.sku_name }}</view>
									<text class="text-right text-[26rpx] font-400 w-[90rpx] leading-[36rpx]">x{{
										item.order_goods.num }}</text>
								</view>
								<view class="text-[26rpx] font-400 leading-[36rpx] text-[#333] mt-[10rpx]">退款状态：{{
						...
						<view class="flex items-center">
							...
							<view class="price-font leading-[42rpx]">
								...
								<text class="text-[32rpx] font-500">{{
									parseFloat(item.apply_money).toFixed(2).split('.')[0] }}</text>
								<text class="text-[22rpx] font-500">.{{
									parseFloat(item.apply_money).toFixed(2).split('.')[1] }}</text>
							</view>
						</view>
					</view>
					<view class="mt-[20rpx] flex flex-wrap justify-end">
						<view
							class="text-[24rpx] text-[#626779] h-[56rpx] leading-[52rpx] box-border px-[23rpx] border-[2rpx] border-solid border-[#8288A2] rounded-full ml-[20rpx]"
							@click.stop="refundBtnFn(item, 'cancel')"
							v-if="['6', '7', '8', '-1'].indexOf(item.status) == -1">
							{{ t('refundApply') }}</view>
						<view v-if="['3'].indexOf(item.status) != -1"
							class="text-[24rpx] text-[#626779] h-[56rpx] box-border  leading-[52rpx] px-[23rpx] border-[2rpx] border-solid border-[#8288A2] rounded-full ml-[20rpx]"
							@click.stop="refundBtnFn(item, 'edit')">编辑退款信息</view>
						<view v-if="['2'].indexOf(item.status) != -1"
							class=" text-[24rpx] text-[#626779] h-[56rpx]  box-border leading-[52rpx] px-[23rpx] border-[2rpx] border-solid border-[#8288A2] rounded-full ml-[20rpx]"
							@click.stop="refundBtnFn(item, 'logistics')">填写发货物流</view>
						<view v-if="['5'].indexOf(item.status) != -1"
							class="text-[24rpx] text-[#626779] h-[56rpx] box-border  leading-[52rpx] px-[23rpx] border-[2rpx] border-solid border-[#8288A2] rounded-full ml-[20rpx]"
...
const refundBtnFn = (data, type) => {
	if (type == 'cancel') {
		...
		redirect({ url: '/addon/phone_shop/pages/refund/detail', param: { order_refund_no: data.order_refund_no, type: 'logistics' } })
	} else if (type == 'editLogistics') {
		redirect({ url: '/addon/phone_shop/pages/refund/detail', param: { order_refund_no: data.order_refund_no, type: 'logistics', is_edit_delivery: true } })
	}
}
