<template>
	<view :style="themeColor()">
		<view class="bg-[#f7f7f7] min-h-[100vh] relative" v-if="Object.keys(goodsDetail).length">
			...
			<view
				...
				<view class="flex items-baseline text-[#fff]">
					<view class="flex items-center">
						<text class="text-[44rpx] leading-[62rpx] price-font"
							v-if="goodsDetail.point">{{ goodsDetail.point }}{{ t('point') }}</text>
						<text class=" text-[36rpx]" v-if="goodsDetail.point && parseFloat(goodsDetail.price)">+</text>
						<text class="text-[44rpx] leading-[62rpx] price-font"
							v-if="parseFloat(goodsDetail.price)">{{ parseFloat(goodsDetail.price).toFixed(2) }}{{ t('priceUnit') }}</text>
					</view>
					<text class="text-[26rpx] line-through ml-[5rpx]"
			...
			<view class="bg-[#f6f6f6] rounded-[16rpx] -mt-[26rpx] relative">
					...
					<view class="border-[2rpx] border-solid border-[#f1f1f1] border-b-0">
						<block v-for="(item, index) in goodsDetail.goods.attr_format" :key="index">
							<view v-if="index < 4 || isAttrFormatShow"
								class="flex border-0 border-solid border-[#f1f1f1] border-b-[2rpx]">
								<view
									class="w-[30%] px-[18rpx] py-[8rpx] text-[24rpx] min-h-[70rpx] break-all box-border flex items-center border-0 border-solid border-[#f1f1f1] border-r-[2rpx] bg-[#fbfafa]">
									{{ item.attr_value_name }}</view>
								<view
									class="w-[70%] px-[18rpx] py-[8rpx] text-[24rpx] min-h-[70rpx] break-all flex items-center box-border">
									{{ Array.isArray(item.attr_child_value_name) ? item.attr_child_value_name.join(',') :
										item.attr_child_value_name }}</view>
							</view>
						</block>
						<view v-if="goodsDetail.goods.attr_format.length > 4"
							class="flex items-center justify-center h-[70rpx] border-0 border-solid border-[#f1f1f1] border-b-[2rpx]"
				...
				<view
					...
					<view class="flex-1" v-if="goodsDetail.goods.status == 1">
						...
						<button v-else
							class="!w-full !h-[72rpx] text-[26rpx] !text-[#fff] !bg-[#ccc] !m-0 leading-[72rpx] rounded-full remove-border">已售罄</button>
					</view>
				...
				<u-popup class="popup-type" :show="servicesDataShow" @close="servicesDataShow = false">
					<view class="min-h-[480rpx] pb-[30rpx]" @touchmove.prevent.stop>
						<view class="flex items-center justify-center py-[34rpx] relative">
							...
							<view class="absolute right-[37rpx]  nc-iconfont nc-icon-guanbiV6xx text-[36rpx]"
								@click="servicesDataShow = false"></view>
						</view>
							...
							<view class="pl-[22rpx] py-[28rpx] pr-[37rpx]">
								<view class="flex mb-[28rpx]" v-for="(item, index) in goodsDetail.service">
									<image class="max-w-[34rpx] max-h-[34rpx] mr-[14rpx]"
										:src="img(item.image || 'addon/phone_shop/icon_service.png')"
										mode="aspectFit" />
									<view class="flex-1">
				...
				<u-popup class="popup-type" :show="distributionDataShow" @close="distributionDataShow = false">
					<view class="min-h-[480rpx] pb-[30rpx]" @touchmove.prevent.stop>
						<view class="flex items-center justify-center py-[34rpx] relative">
							...
							<view class="absolute right-[37rpx]  nc-iconfont nc-icon-guanbiV6xx text-[36rpx]"
								@click="distributionDataShow = false"></view>
						</view>
						<scroll-view class="h-[520rpx]" scroll-y="true">
			...
			<share-poster ref="sharePosterRef" posterType="shop_point_goods" :posterParam="posterParam"
