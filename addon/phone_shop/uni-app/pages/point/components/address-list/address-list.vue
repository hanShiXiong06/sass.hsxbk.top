	...
	<u-popup :show="show" @close="show = false" mode="bottom" :round="10" :closeable="true">
		<view @touchmove.prevent.stop>
			...
			<scroll-view scroll-y="true" class="max-h-[50vh]">
				<view v-for="(item, index) in addressList" :key="item.id" class="flex items-center px-[30rpx]"
					:class="{ 'mb-[30rpx]': addressList.length - 1 != index, 'text-[var(--primary-color)]': item.id == currAddressId }"
...
const editAddress = (data: Object, isSelectMap: number = 2) => {
	uni.setStorage({
		...
		data: {
			back: '/addon/phone_shop/pages/point/payment',
