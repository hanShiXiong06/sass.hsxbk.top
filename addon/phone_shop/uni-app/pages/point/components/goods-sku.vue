	...
	<view @touchmove.prevent.stop>
		<u-popup class="popup-type" :show="goodsSkuPop" @close="closeFn" mode="bottom">
			<view class="p-[32rpx] relative" v-if="goodsDetail.detail" @touchmove.prevent.stop>
				<view class="absolute right-[37rpx]  nc-iconfont nc-icon-guanbiV6xx text-[36rpx]" @click="closeFn">
				</view>
				<view class="flex mb-[58rpx]">

					<view class="rounded-[8rpx] overflow-hidden">
						<u--image width="180rpx" height="180rpx" :src="img(goodsDetail.detail.sku_image)"
							@click="imgListPreview(goodsDetail.detail.sku_image)" model="aspectFill">
							<template #error>
					...
					<view class="flex flex-1 flex-col ml-[20rpx] py-[10rpx]">
						<view class="w-[100%]">
							<view class="text-[var(--price-text-color)] flex items-baseline">
								...
								<text class="text-[36rpx] mb-[10rpx]"
									v-if="goodsDetail.point && parseFloat(goodsDetail.price)">+</text>
								<template v-if="parseFloat(goodsDetail.price)">
									...
									<text class="text-[48rpx] price-font">{{
										parseFloat(goodsDetail.price).toFixed(2).split('.')[0] }}</text>
									<text class="text-[32rpx] mr-[6rpx] price-font">.{{
										parseFloat(goodsDetail.price).toFixed(2).split('.')[1] }}</text>
								</template>
							...
							<view class="text-[24rpx] leading-[32rpx] text-[#303133] mt-[12rpx]">
								库存{{ goodsDetail.detail.stock }}{{
									goodsDetail.goods.unit }}</view>
						</view>
						<view class="w-[100%] mt-auto" style="max-height: calc(204rpx - 98rpx); overflow: hidden;"
							v-if="goodsDetail.goodsSpec && goodsDetail.goodsSpec.length">
							<text
								class="text-[24rpx] leading-[30rpx] text-[#666]">已选规格：{{ goodsDetail.detail.sku_spec_format }}</text>
						</view>
					</view>
				</view>
				<scroll-view class="h-[500rpx] mb-[30rpx]" scroll-y="true">
					...
					<view class="flex justify-between items-center mt-[30rpx]">
						...
						<u-number-box :min="1"
							...
							integer :step="1" input-width="98rpx" v-model="buyNum" input-height="54rpx">
							<template #minus>
							...
							<template #plus>
								<text class="text-[34rpx] nc-iconfont nc-icon-jiahaoV6xx"
									:class="{ '!text-[#c8c9cc]': buyNum === goodsDetail.stock || buyNum == parseInt(goodsDetail.detail.limit_num) }"></text>
							</template>
						</u-number-box>
					</view>
				</scroll-view>
				<button v-if="goodsDetail.detail.stock > 0"
...
const confirm = () => {
	...
	uni.setStorage({
		...
		success: () => {
			redirect({ url: '/addon/phone_shop/pages/point/payment' })
		}
	});
