		...
		<view class="fixed left-0 right-0 top-0 product-warp bg-[#fff] px-[30rpx] z-10">
			<view class="py-[14rpx] flex items-center justify-between">
				<view class="flex-1 flex items-center h-[64rpx] bg-[#F6F8F8] rounded-[33rpx] px-[32rpx]">
					<u-input class="flex-1" maxlength="50" v-model="goods_name"
						@confirm="searchTypeFn('total_order_num')" placeholder="请搜索您想要的商品"
						placeholderClass="text-[#a5a6a6] text-[26rpx]" fontSize="26rpx" clearable
						border="none"></u-input>
					<text class="nc-iconfont nc-icon-sousuo-duanV6xx1 text-[32rpx] ml-[18rpx] !text-[#999]"
						@click="searchTypeFn('total_order_num')"></text>
				</view>
			...
			<view class="flex justify-between tems-center py-[22rpx] px-[20rpx]">
				<view class=" flex items-center justify-between text-[24rpx] text-[#666] flex-1">
					<text :class="{ 'text-[#303133] ': searchType == 'total_order_num' }"
						@click="searchTypeFn('total_order_num')">综合排序</text>
					<view class="flex items-center" :class="{ 'text-[#303133]': searchType == 'total_exchange_num' }"
						@click="searchTypeFn('total_exchange_num')">
						<text class="mr-[4rpx]">销量</text>
						<text v-if="(sale_num != 'asc') && (sale_num != 'desc')"
							class="text-[30rpx] text-[#666] nc-iconfont nc-icon-shangxiaxuanzeV6xx"></text>
						<text v-else-if="sale_num == 'asc'"
							class="text-[18rpx] w-[30rpx] h-[30rpx] flex items-center justify-center text-[#666] nc-iconfont font-bold nc-icon-shangV6xx-1 font-bold"></text>
						<text v-else-if="sale_num == 'desc'"
							class="text-[18rpx] w-[30rpx] h-[30rpx] flex items-center justify-center text-[#666] font-bold nc-iconfont nc-icon-xiaV6xx"></text>
					</view>
					<view class="flex items-center" :class="{ 'text-[#303133]': searchType == 'price' }"
						@click="searchTypeFn('price')">
						<text class=" mr-[4rpx]">价格</text>
						<text v-if="(price != 'asc') && (price != 'desc')"
							class="text-[30rpx] text-[#666] nc-iconfont nc-icon-shangxiaxuanzeV6xx"></text>
						<text v-else-if="price == 'asc'"
							class="text-[18rpx] w-[30rpx] h-[30rpx] flex items-center justify-center font-bold text-[#666] nc-iconfont nc-icon-shangV6xx-1 font-bold"></text>
						<text v-else-if="price == 'desc'"
							class="text-[18rpx] w-[30rpx] h-[30rpx] flex items-center justify-center font-bold text-[#666] nc-iconfont nc-icon-xiaV6xx"></text>
					</view>
				</view>
			</view>
		</view>
		...
		<mescroll-body ref="mescrollRef" top="160rpx" bottom="50px" @init="mescrollInit" :down="{ use: false }"
			...
			<view v-if="goodsList.length" class="sidebar-marign flex justify-between flex-wrap">
				<template v-for="(item, index) in goodsList">
					<view
						class="goods-item-style-two flex flex-col bg-[#fff] box-border rounded-[16rpx] overflow-hidden mt-[20rpx]"
						...
						<view class="px-[16rpx] flex-1 pt-[10rpx] pb-[20rpx] flex flex-col justify-between">
							...
							<view class="text-[24rpx] font-400 leading-[34rpx] mt-[10rpx] text-[#999]">
								已兑{{ item.total_exchange_num }}人</view>
							<view class="flex justify-between flex-wrap items-center mt-[16rpx]">
								<view class="flex items-baseline">
									<image class="h-[28rpx] self-center"
										:src="img('addon/phone_shop/exchange/point-icon.png')" mode="heightFix" />
									<text class="text-[var(--price-text-color)] price-font text-[32rpx] ml-[2rpx]">{{
										item.point }}</text>
									<text class="text-[#333] font-400 text-[24rpx]"
										v-if="item.price && item.price > 0">+</text>
									<text class="text-[var(--price-text-color)] font-400 text-[22rpx]"
										v-if="item.price && item.price > 0">￥{{
											parseFloat(item.price).toFixed(2) }}</text>
								</view>
								<view
									class="w-[80rpx] h-[46rpx] text-[28rpx] leading-[46rpx] !text-[#fff] m-0 rounded-full !bg-[var(--primary-color)] font-400 remove-border text-center"
									shape="circle">兑换</view>
							</view>
						</view>
					</view>
				</template>
			</view>
			<view class="mx-[30rpx] mt-[20rpx]  rounded-[16rpx] noData flex items-center justify-center"
