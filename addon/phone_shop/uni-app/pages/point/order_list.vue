		...
		<view class="fixed left-0 top-0 right-0 z-10" v-if="statusLoading">
			<scroll-view scroll-x="true" class="scroll-Y box-border px-[30rpx] bg-white">
				<view class="flex whitespace-nowrap justify-between">
					<view
						:class="['text-[28rpx] leading-[90rpx]', { 'class-select': orderState === item.status.toString() }]"
					...
					<view class="mb-[20rpx] bg-[#fff] p-[20rpx] rounded-[16rpx]">
						<view @click="toLink(item)">
							<view class="flex justify-between items-center">
								<view class="text-[#303133] text-[28rpx] font-400 leading-[30rpx]">{{ t('orderNo') }}：{{
									item.order_no }}</view>
								<view class="text-[#EF900A] text-[28rpx]">{{ item.status_name.name }}</view>
							</view>
							<view class="flex box-border mt-[30rpx]" v-for="(subitem, index) in item.order_goods"
								:key="index">
								...
								<view class="ml-[20rpx] flex flex-1 flex-col justify-between box-border">
									<view>
										<text class="text-[28rpx] text-item  leading-[40rpx] text-[#303133]">{{
											subitem.goods_name }}</text>
										<view v-if="subitem.sku_name">
											<view
												class="text-[24rpx] truncate mt-[10rpx] text-[#999] leading-[28rpx] max-w-[480rpx]">
												{{
													subitem.sku_name }}</view>
										</view>
									</view>
										...
										<view class="text-right leading-[28rpx] price-font">
											<block v-if="item.activity_type == 'exchange'">
												...
												<block v-if="parseFloat(subitem.price)">
													...
													<text
														class="text-[32rpx] font-500">{{ parseFloat(subitem.price).toFixed(2).split('.')[0]
														}}</text>
													<text
														class="text-[22rpx] font-500">.{{ parseFloat(subitem.price).toFixed(2).split('.')[1]
														}}</text>
												</block>
											</block>
											<block v-else>
									...
									<block v-if="item.activity_type == 'exchange'">
										...
										<block v-if="parseFloat(item.order_money)">
											...
											<text class="text-[24rpx] font-500">.{{
												parseFloat(item.order_money).toFixed(2).split('.')[1]
												}}</text>
										</block>
									</block>
									<block v-else>
