	...
	<view :style="themeColor()">
		<view class="cate-search">
			<view class="search-box">
				<input class="uni-input text-[24rpx]" maxlength="50" v-model="inputValue" confirm-type="search" focus
					@confirm="search()" placeholder="搜索商品" />
				<u-icon v-if="inputValue" name="close-circle-fill" color="#A5A6A6" size="28rpx"
					@click="inputValue = ''"></u-icon>
			...
			<view class="history" v-if="historyList.length">
				<view class="history-box">
					<view class="history-top">
						...
						<view class="icon nc-iconfont nc-icon-shanchu-yuangaizhiV6xx text-[28rpx] text-[#999]"
							@click="deleteHistoryList"></view>
					</view>
...
const search = () => {
	if (inputValue.value.trim() != '') {
		...
		redirect({ url: '/addon/phone_shop/pages/goods/list', param: { goods_name: inputValue.value.trim() }, mode: 'navigateTo' })
	}
}
