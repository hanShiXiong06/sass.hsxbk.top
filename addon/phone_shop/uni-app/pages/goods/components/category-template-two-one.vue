	...
	<view class="bg-[#F4F6F8] min-h-screen">
		<view class="mescroll-box" v-if="tabsData.length">
			...
			<scroll-view class="h-[100vh]" :scroll-y="true">
				<view class=" pl-[202rpx] scroll-ios pt-[20rpx] pr-[20rpx]"
					...
					<view
						...
						<template v-for="(item, index) in tabsData[tabActive]?.child_list" :key="item.category_id">
							<view class="" @click="toLink(item.category_id)">
								<u--image radius="12rpx" width="129rpx" height="129rpx"
									:src="img(item.image ? item.image : '')" model="aspectFill">
									<template #error>
										<image class="rounded-[12rpx] overflow-hidden w-[129rpx] h-[129rpx]"
											:src="img('static/resource/images/diy/shop_default.jpg')" mode="aspectFill">
										</image>
									</template>
								</u--image>
								<view class="text-[24rpx] text-center mt-[12rpx] leading-[34rpx]">{{ item.category_name
									}}</view>
							</view>
						</template>
					</view>
					<view class="flex justify-center w-[100%] rounded-[16rpx] flex items-center"
						:class="{ 'noData1': config.search.control, 'noData2': !(config.search.control) }"
						v-if="!tabsData[tabActive]?.child_list && !loading">
		...
		<tabbar />
		<view class="flex justify-center w-[100%] items-center justify-center h-[100vh]"
			v-if="!tabsData.length && !loading">
			<mescroll-empty :option="{ tip: '暂无商品分类' }"></mescroll-empty>
		</view>
