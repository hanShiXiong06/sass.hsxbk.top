<template>
	<view class="bg-[#F6F6F6] min-h-[100vh] overflow-hidden" :style="themeColor()">
		<view class="coupon-header fixed  left-0 right-0 top-0 z-10080">
			<!-- #ifdef MP-WEIXIN -->
			<view
				:style="{ height: headStyle, backgroundImage: 'url(' + img('addon/phone_shop/coupon/coupon_uniapp.png') + ')', backgroundSize: '100%', backgroundPosition: 'bottom', backgroundRepeat: 'no-repeat' }">
				<top-tabbar :data="param" class="top-header" />
			</view>
			<!-- #endif -->
			<!-- #ifdef H5 -->
			<view class="h-[364rpx]"
				:style="{ backgroundImage: 'url(' + img('addon/phone_shop/coupon/coupon_uniapp.png') + ')', backgroundSize: 'cover', backgroundPosition: 'bottom', backgroundRepeat: 'no-repeat' }">
			</view>
			<!-- #endif -->
			<view
				class="-mt-[-36rpx] px-[var(--sidebar-m)] py-[24rpx] flex items-center justify-between leading-[40rpx] text-[28rpx] bg-[#F6F6F6] rounded-t-[26rpx] relative z-99999"
				:class="{ '!bg-[#fff]': typePopup }">
				<text :class="{ 'text-primary ': searchType == 'all' }" @click="searchTypeFn('all')">默认排序</text>
				<view class="flex items-center" :class="{ 'text-primary': searchType == 'create_time' }"
					@click="searchTypeFn('create_time')">
					<text class="mr-[4rpx]">最新</text>
					<text v-if="create_time == 'asc'"
						class="text-[18rpx] text-[#626779] nc-iconfont nc-icon-a-xiangshangV6xx1"
						:class="{ '!text-primary': searchType == 'create_time' }"></text>
					<text v-else class="text-[18rpx] text-[#626779]  nc-iconfont nc-icon-a-xiangxiaV6xx1"
						:class="{ '!text-primary': searchType == 'create_time' }"></text>
				</view>
				<view class="flex items-center" :class="{ 'text-primary': searchType == 'price' }"
				...
				<view class="flex items-center" :class="{ 'text-primary': searchType == 'type' }"
		...
		<mescroll-body ref="mescrollRef" @init="mescrollInit" :down="{ use: false }" height="auto"
			@up="getShopCouponListFn" :top="mescrollTop">
			<view class="py-[20rpx] sidebar-marign">
				<template v-for="(item, index) in list">
					<view v-if="item.btnType === 'collected'"
						...
						<view
							...
							<view class="w-[164rpx] box-border flex justify-center ">
								<view class="flex items-baseline text-[var(--price-text-color)]">
									...
									<text
										class="text-[54rpx] font-500 text-left leading-[70rpx] max-w-[136rpx] truncate price-font">{{
							...
							<view class="flex-1 box-border ml-[10rpx]">
								...
								<view class="mt-[10rpx] text-left flex items-center">
									<text
										class="bg-[#FFEFF0] whitespace-nowrap text-[#EF000C] text-[20rpx] h-[28rpx] leading-[28rpx] px-[10rpx] rounded-[13rpx] mr-[10rpx] flex-shrink-0">{{
											item.type_name }}</text>
									<text class="text-[22rpx] truncate max-w-[190rpx] leading-[30rpx] text-[#626779]">{{
										item.title }}</text>
								</view>
						...
						<view class="pr-[20rpx] pl-[34rpx]">
							<button
								:style="{ width: '144rpx', height: '56rpx', color: '#fff', fontSize: '24rpx', lineHeight: '56rpx', padding: '0', backgroundColor: '#ee9ea2', border: 'none', opacity: '1', borderRadius: '30rpx' }"
								disabled>已领完</button>
						</view>
						<view
							class="absolute top-0 right-[190rpx]  h-[10rpx] w-[20rpx] rounded-br-[20rpx] rounded-bl-[20rpx] bg-[#F6F6F6] ">
						</view>
						<view
							class="absolute bottom-0 right-[190rpx] h-[10rpx] w-[20rpx] rounded-tr-[20rpx] rounded-tl-[20rpx] bg-[#F6F6F6]">
						</view>
					</view>
					<view v-else
						class="flex items-center relative w-[100%] rounded-[20rpx] overflow-hidden bg-[#fff] py-[20rpx] background-size"
						...
						<view
							...
							<view class="flex-1 box-border ml-[10rpx]">
								...
								<view class="mt-[10rpx] text-left flex items-center">
									<text
										class="bg-[#FFEFF0] whitespace-nowrap text-[#EF000C] text-[20rpx] h-[28rpx] leading-[28rpx] px-[10rpx] rounded-[14rpx] mr-[10rpx] flex-shrink-0">{{
											item.type_name }}</text>
									<text class="text-[22rpx] truncate max-w-[190rpx] leading-[30rpx] text-[#626779]">{{
										item.title }}</text>
								</view>
						...
						<view v-if="item.btnType === 'collecting'" @click.stop="collecting(item.id, index)"
							...
							<button
								:style="{ width: '144rpx', height: '56rpx', color: '#fff', fontSize: '24rpx', lineHeight: '56rpx', padding: '0', backgroundColor: 'var(--primary-color)', border: 'none', borderRadius: '30rpx' }">立即领取</button>
						</view>
						<view v-if="item.btnType === 'using'" @click.stop="toLink(item.id)"
							class="pr-[20rpx] pl-[34rpx]">
							<button
								:style="{ width: '144rpx', height: '56rpx', color: 'var(--primary-color)', fontSize: '24rpx', lineHeight: '52rpx', padding: '0', backgroundColor: 'transparent', border: '2rpx solid var(--primary-color)', borderRadius: '30rpx' }">去使用</button>
						</view>
						<view
							class="absolute top-0 right-[190rpx]  h-[10rpx] w-[20rpx] rounded-br-[20rpx] rounded-bl-[20rpx] bg-[#F6F6F6] ">
						</view>
				...
				<view v-if="!list.length && !loading" :style="{ 'height': nullPageHeight }"
					class="noData  rounded-[20rpx] flex items-center justify-center">
			...
			<view @touchmove.prevent.stop>
				<scroll-view :scroll-x="true" scroll-with-animation
					...
					<view class="items-center flex py-[25rpx]  border-0 border-t-[2rpx] border-solid border-[#F0F2F8]">
						<text
							class="flex-shrink-0 w-[120rpx] h-[50rpx] text-[26rpx] leading-[50rpx] text-center text-[#626779] bg-[#F8F9FD] rounded-[30rpx] border-box mr-[20rpx] border-[2rpx] border-solid border-[#F8F9FD]"
							:class="{ '!text-primary !border-primary !bg-[rgba(239,0,12,0.04)] font-500': item.value == curType }"
