...
import {
  ...
  getCartList,
  getCartSum,
} from "@/addon/phone_shop/api/cart";
  ...
  actions: {
        ...
        if (data) {
          data.forEach((item: any) => {
            if (
              ...
              if (
                item.goods.is_discount &&
                item.goodsSku.sale_price != item.goodsSku.price
              ) {
                cart.sale_price = item.goodsSku.sale_price
                  ? item.goodsSku.sale_price
                  : item.goodsSku.price; // 折扣价
              } else if (
                item.goods.member_discount &&
                getToken() &&
                item.goodsSku.member_price != item.goodsSku.price
              ) {
                cart.sale_price = item.goodsSku.member_price
                  ? item.goodsSku.member_price
                  : item.goodsSku.price; // 会员价
              }
              ...
              this.cartList["goods_" + cart.goods_id]["sku_" + cart.sku_id] =
                cart;
            }
          });
        }
    ...
    reduce(data: any, step = 1, callback: any = null) {
      ...
      if (updateNum > 0) {
        ...
        if (Object.keys(this.cartList["goods_" + data.goods_id]).length == 0)
        ...
        for (let goods in this.cartList) {
          ...
          for (let sku in this.cartList[goods]) {
            if (typeof this.cartList[goods][sku] == "object") {
              ...
              totalMoney +=
                this.cartList[goods][sku].num *
                this.cartList[goods][sku].sale_price;
            }
          }
