		...
		<view :style="warpCss">
			...
			<div class="diy-shop-goods-list relative flex flex-wrap justify-between">
				<block v-if="diyComponent.style == 'style-1'">
					<view class="bg-white w-full flex p-[20rpx] mx-[20rpx] rounded-[8rpx] overflow-hidden"
						...
						<u--image radius="10rpx" width="190rpx" height="190rpx"
						...
						<view class="flex-1 flex flex-col ml-[20rpx] py-[6rpx]">
							...
							<view class="text-[20rpx] leading-[40rpx] text-[#303133] multi-hidden mb-[10rpx]"
								:style="{ color: diyComponent.goodsNameStyle.color, fontWeight: diyComponent.goodsNameStyle.fontWeight }">
								{{ item.sub_title }}</view>
							<view class="mt-auto flex justify-between items-center items-baseline">
								<view class="font-bold text-[var(--price-text-color)] price-font flex items-baseline"
								...
								<!-- <text class="text-[22rpx] text-[#999]"
									:style="{ color: diyComponent.saleStyle.color }">已售{{ item.sale_num }}{{
										item.unit ||
									'件'}}</text> -->
							</view>
						</view>
				...
				<block v-if="diyComponent.style == 'style-2'">
					<view class="flex flex-col bg-[#fff] box-border rounded-[12rpx] overflow-hidden"
						:class="{ 'mt-[24rpx]': index > 1 }" :style="itemCss" v-for="(item, index) in goodsList"
						:key="item.goods_id" @click="toLink(item)">
						<u--image :width="style2Width" :height="style2Width"
							:src="img(item.goods_cover_thumb_mid || '')" model="aspectFill">
							<template #error>
								<image :style="{ 'width': style2Width, 'height': style2Width }"
									:src="img('static/resource/images/diy/shop_default.jpg')" mode="aspectFill"></image>
							</template>
						</u--image>
						<view class="px-[16rpx] flex-1 pt-[16rpx] pb-[20rpx] flex flex-col justify-between">
							...
							<view class="flex justify-between flex-wrap items-baseline mt-[16rpx]">
								...
								<!-- <text class="text-[22rpx] text-[#999]"
									:style="{ color: diyComponent.saleStyle.color }">已售{{
				...
				<block v-if="diyComponent.style == 'style-3'">
					<view :style="style3Css">
						<scroll-view :id="'warpStyle3-' + diyComponent.id" class="whitespace-nowrap h-[284rpx]"
							:scroll-x="true">
							<view :id="'item' + index + diyComponent.id"
								class="w-[214rpx] rounded-[10rpx] inline-block bg-[#fff] box-border overflow-hidden"
								...
								<u--image width="214rpx" height="160rpx" :src="img(item.goods_cover_thumb_mid || '')"
									model="aspectFill">
									<template #error>
										<image class="w-[214rpx] h-[160rpx]"
								...
								<view class="px-[10rpx] pt-[16rpx] pb-[10rpx]">
									<view class="text-[26rpx] text-[#303133] truncate"
										:style="{ color: diyComponent.goodsNameStyle.color, fontWeight: diyComponent.goodsNameStyle.fontWeight }">
										{{ item.goods_name }} </view>
									<view class="text-[18rpx] text-[#303133] truncate"
										:style="{ color: diyComponent.goodsNameStyle.color, fontWeight: diyComponent.goodsNameStyle.fontWeight }">
										{{ item.sub_title }}</view>
									<view
										class="text-[var(--price-text-color)] py-[10rpx] flex-wrap font-bold price-font flex items-baseline leading-[1]"
										:style="{ color: diyComponent.priceStyle.mainColor }">
										<view class="truncate">
											<text class="text-[26rpx] font-500">￥</text>
											<text class="text-[36rpx] font-500">{{
