			...
			<template v-if="diyComponent.headStyle == 'style-3'">
				<template v-if="diyComponent.source == 'custom'">
					<view v-for="(item, index) in diyComponent.list" :key="index"
						...
						<text
							:class="['text-[24rpx] mt-[14rpx]', { 'text-[var(--primary-color)]': index == cateIndex }]">{{
								item.title }}</text>
					</view>
				</template>
				<template v-else-if="diyComponent.source == 'goods_category'">
					<view class="pr-[40rpx] inline-flex flex-col items-center justify-center"
						...
						<text
							:class="['text-[24rpx] mt-[14rpx]', { 'text-[var(--primary-color)]': currentCategoryId == 0 }]">全部</text>
					</view>
					<view v-for="(item, index) in goodsCategoryListData" :key="index"
						...
						<image :style="{ borderRadius: (diyComponent.aroundRadius * 2) + 'rpx' }"
							...
							v-else :src="img('static/resource/images/diy/figure.png')" mode="scaleToFill" />
						<text
							:class="['text-[24rpx] mt-[14rpx]', { 'text-[var(--primary-color)]': currentCategoryId == item.category_id }]">{{
...
import diyGoodsList from '@/addon/phone_shop/components/diy/phone-goods-list/index.vue';
