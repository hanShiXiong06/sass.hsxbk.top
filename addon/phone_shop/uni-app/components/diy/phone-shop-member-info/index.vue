	...
	<view :style="warpCss">
		<view class="px-[30rpx] pt-[30rpx] pb-[120rpx] box-border">
			...
			<view v-if="info" class="flex items-center">
				<!-- 唤起获取微信 -->
				<u-avatar :src="img(info.headimg)" :size="'100rpx'" leftIcon="none"
					:default-url="img('static/resource/images/default_headimg.png')" @click="clickAvatar"></u-avatar>
				<view class="ml-[20rpx] flex-1">
					<view class="text-[#ffffff] flex items-center flex-wrap"
						:style="{ color: diyComponent.textColor }">
						<view class="text-[32rpx] truncate max-w-[320rpx] font-bold leading-[38rpx]">{{ info.nickname }}
						</view>
						<view class="text-[24rpx] font-400 leading-[28rpx] ml-[10rpx]" v-if="info.mobile">
							{{ info.mobile.replace(info.mobile.substring(3, 7), "****") }}</view>
						<view v-else-if="!info.mobile" @click="bindMobileFn"
							class="text-[22rpx] text-[#626779] ml-[10rpx] px-[6rpx] border-[1rpx] border-solid border-[#E3E4E9] rounded-[8rpx] leading-[30rpx]">
							{{ t('bindMobile') }}</view>
					</view>
					<view class="text-[#ffffff] text-[24rpx] font-400 leading-[28rpx] mt-[6rpx]"
						:style="{ color: diyComponent.textColor }">UID：{{ info.member_no }}</view>
				</view>
			...
			<view v-else class="flex items-center" @click="toLogin">
				<u-avatar src="" :size="'100rpx'"
					:default-url="img('static/resource/images/default_headimg.png')"></u-avatar>
				<view class="ml-[20rpx] flex-1">
			...
			<view class="flex mt-[30rpx] items-center">
				<view class="text-center w-[33.333%] flex-shrink-0">
					<view class="text-[36rpx] leading-[42rpx] h-[50rpx] font-500">
						<view @click="redirect({ url: '/app/pages/member/balance' })"
							:style="{ color: diyComponent.textColor }">{{ money }}</view>
					</view>
					<view class="text-[22rpx] leading-[26rpx] h-[31rpx] font-400">
						<view @click="redirect({ url: '/app/pages/member/balance' })"
							:style="{ color: diyComponent.textColor }">{{ t('balance') }}</view>
					</view>
				</view>
				<view class="text-center w-[33.333%] flex-shrink-0">
					<view class="text-[36rpx] leading-[42rpx] h-[50rpx] font-500">
						<view @click="redirect({ url: '/app/pages/member/point' })"
							:style="{ color: diyComponent.textColor }">{{ parseInt(info?.point) || 0 }}</view>
					</view>
					<view class="text-[22rpx] leading-[26rpx] h-[31rpx] font-400">
						<view @click="redirect({ url: '/app/pages/member/point' })"
							:style="{ color: diyComponent.textColor }">{{ t('point') }}</view>
					</view>
				</view>
				<view class="text-center w-[33.333%] flex-shrink-0"
					@click="redirect({ url: '/addon/phone_shop/pages/member/my_coupon' })">
					<view class="text-[36rpx] leading-[42rpx] h-[50rpx] font-500">
