				...
				<view class="p-[32rpx] relative" v-if="goodsDetail.detail" @touchmove.prevent.stop>
					<view class="flex mb-[58rpx]">
						...
						<view class="flex flex-1 flex-col ml-[20rpx] py-[10rpx]">
							<view class="w-[100%]">
								<view class="text-[var(--price-text-color)] flex items-baseline">
									...
									<text class="text-[48rpx] price-font">{{
										parseFloat(goodsPrice).toFixed(2).split('.')[0] }}</text>
							...
							<view class="w-[100%] mt-auto"
								style="max-height: calc(204rpx - 98rpx); overflow: hidden; box-sizing:border-box"
								v-if="goodsDetail.goodsSpec && goodsDetail.goodsSpec.length">
								<text
									class="text-[24rpx] leading-[30rpx] text-[#666] flex items-center h-[60rpx] mt-[10rpx]">已选规格：{{
										goodsDetail.detail.sku_spec_format }}</text>
							</view>
					...
					<scroll-view class="h-[500rpx] mb-[30rpx]" scroll-y="true">
						...
						<view class="flex justify-between items-center mt-[30rpx]">
							...
							<u-number-box :min="1" :max="goodsDetail.stock" integer :step="1" input-width="68rpx"
								v-model="buyNum" input-height="52rpx">
								<template #minus>
					...
					<button v-if="goodsDetail.detail.stock > 0"
						class="!h-[72rpx] leading-[72rpx] text-[26rpx] rounded-[50rpx] primary-btn-bg" type="primary"
