            ...
            <div class="mt-[10px]">
                <el-table :data="tableData.data" size="large" v-loading="tableData.loading">
                    ...
                    <el-table-column :label="t('sumCount')" min-width="160">
                        <template #default="{ row }">
                            <span v-if="row.sum_count != '-1'">{{ row.remain_count || '' }} / {{ row.sum_count || ''
                                }}</span>
                    ...
                    <el-table-column :label="t('operation')" fixed="right" align="right" min-width="160">
                        <template #default="{ row }">
                            ...
                            <el-button type="primary" link @click="editEvent(row)" v-if="row.status == 1">{{ t('edit')
                                }}</el-button>
...
import { getCouponList, deleteCoupon, closeCoupon } from '@/addon/phone_shop/api/marketing'
import { ElMessageBox, FormInstance } from 'element-plus'
import { t } from '@/lang'
import couponSpreadPopup from '@/addon/phone_shop/views/marketing/coupon/components/coupon-spread-popup.vue'
...
const collectionEvent = (data: any) => {
    router.push('/phone_shop/marketing/coupon/collection?id=' + data.id)
}
