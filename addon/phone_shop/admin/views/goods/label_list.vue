            ...
            <el-card class="box-card !border-none my-[10px] table-search-wrap" shadow="never">
                <el-form :inline="true" :model="labelTable.searchParam" ref="searchFormRef">
                    <el-form-item :label="t('labelName')" prop="label_name">
                        <el-input v-model.trim="labelTable.searchParam.label_name"
                            :placeholder="t('labelNamePlaceholder')" />
                    </el-form-item>
                    <el-form-item :label="t('groupName')" prop="group_id">
                        <el-select v-model="labelTable.searchParam.group_id" :placeholder="t('groupNamePlaceholder')"
                            clearable>
                ...
                <el-table :data="labelTable.data" size="large" v-loading="labelTable.loading" @sort-change="sortChange">
                    ...
                    <el-table-column prop="label_name" :label="t('label')" min-width="120">
                        <template #default="{ row }">
                            ...
                            <img v-else-if="row.style_type == 'icon'" class="block h-[28px] rounded-[4px] object-cover"
                                :src="img(row.icon)" />
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="status" :label="t('status')" min-width="80" :show-overflow-tooltip="true">
                        <template #default="{ row }">
                            <el-tag type="success" v-if="row.status == 1"
                                @click="modifyLabelStatusEvent(row.label_id, 0)" class="cursor-pointer">{{ t('statusOn')
                                }}</el-tag>
                            <el-tag type="info" v-else @click="modifyLabelStatusEvent(row.label_id, 1)"
                                class="cursor-pointer">{{ t('statusOff') }}</el-tag>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column :label="t('operation')" fixed="right" align="right" min-width="120">
                        <template #default="{ row }">
                            ...
                            <el-button type="primary" v-if="userStore().siteInfo.site_id == row.site_id" link
                                @click="deleteEvent(row.label_id)">{{ t('delete')
                                }}</el-button>
                        </template>
                    </el-table-column>
...
import { t } from '@/lang'
import { getLabelPageList, deleteLabel, modifyLabelSort, getLabelGroupList, modifyLabelStatus } from '@/addon/phone_shop/api/goods'
