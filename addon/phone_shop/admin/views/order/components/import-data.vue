...
const examineTemplate = ()=>{
    let url = `${import.meta.env.VITE_IMG_DOMAIN || location.origin}/addon/phone_shop/batch/整单发货模板.xls`;
    if(importData.value.type == 'order_goods'){
        url = `${import.meta.env.VITE_IMG_DOMAIN || location.origin}/addon/phone_shop/batch/拆单发货模板.xls`;
    }
    window.open(url)
}
