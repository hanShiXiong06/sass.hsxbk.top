...
class  CoreStatService extends BaseCoreService
{
    ...
    public static function addStat(int $site_id, $data = []) {
        ...
        $stat = (new TourismStat())->where($stat_data)->findOrEmpty();
        if ($stat->isEmpty()) {
        ...
        $stat_hour = (new TourismHourStat())->where($stat_hour_data)->findOrEmpty();
    ...
    public function getStatData(int $site_id, string $start_date, string $end_date) {
        ...
        $stat_data = (new TourismStat())->where([ ['site_id', '=', $site_id], ['date_time', '>=', $start_date], ['date_time', '<=', $end_date] ])->field($field)->select()->toArray();
    ...
    public function getHourStatData(int $site_id, string $date){
        ...
        $stat_data = (new TourismHourStat())->where([ ['site_id', '=', $site_id], ['date', '=', $date] ])->field($field)->select()->toArray();
    ...
    public function getStat(int $site_id, string $start_date = '', string $end_date = '') {
        ...
        $stat_data = (new TourismStat())->where($condition)->field(implode(',', $field))->find()->toArray();
