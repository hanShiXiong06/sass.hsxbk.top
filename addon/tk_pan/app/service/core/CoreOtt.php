...
namespace addon\tk_pan\app\service\core;
 ...
 * 123盘存储引擎
 */
class CoreOtt extends BaseUpload
{
    ...
    public function upload(string $dir)
    {
        ...
        return $this->commonUpload($filePath,$fullPath);
    }
...
//            throw new UploadFileException(100013);
    ...
    public function http_get(string $api, array $query_params = [])
    {
        ...
        if (isset($responseData['code']) && $responseData['code'] === 0) {
            ...
            throw new \Exception("API Error: HTTP Code {$httpCode}. {$api}. Response: {$response}");
        }
    }
    ...
    public function http_post(string $api, array $data)
    {
        ...
        if ($responseData['message'] == 'ok') {
            ...
            throw new \Exception("HTTP Code: {$httpCode}.{$api}.Response: {$response}");
        }
    }
