            ...
            <view v-if="isVerifier" class="bg-[#f7f7f7] min-h-screen overflow-hidden">
                ...
                <view v-if="verifyDetail" class="mx-[30rpx]">
                    <view class="bg-white px-[30rpx] py-[40rpx] rounded">
                        ...
                        <view class="text-center font-bold text-[#f00] mt-[10rpx] text-sm">{{ isCanVerify ? t('waitUse')
                            : t('used') }}</view>
                    </view>
                    <view class="bg-white px-[30rpx] py-[40rpx] rounded flex mt-[20rpx]">
                        <view class="w-[180rpx] mr-3 overflow-hidden rounded leading-none">
                            <image :src="img(verifyDetail.cover_thumb_small)" mode="widthFix"
                                class="w-full leading-none"></image>
                        </view>
                    ...
                    <view class="bg-white px-[30rpx] py-[30rpx] rounded mt-[20rpx]">
                        ...
                        <view class="flex text-sm mt-[20rpx]">
                            ...
                            <view>{{ verifyDetail.expire_time ? $u.timeFormat(verifyDetail.expire_time, `yyyy-mm-dd
                                hh:MM: ss`) : t('longTerm') }}</view>
                        </view>
                        <view class="flex text-sm mt-[20rpx]">
                            <view class="text-gray-400 w-[150rpx]">{{ t('availableNum') }}：</view>
                            <view v-if="verifyDetail.card.card_type == 'timecard'">{{ t('noLimitNum') }}</view>
                            <view v-else>{{ verifyDetail.card.card_type == 'oncecard' ? verifyDetail.num :
                                verifyDetail.card.total_num }}</view>
                        </view>
                        <view class="flex text-sm mt-[20rpx]">
                    ...
                    <view class="mt-[30rpx]" v-if="isCanVerify">
                        <button
                            ...
                            @click="handleVerify">{{ t('confirmVerify') }}</button>
                    </view>
                    <view class="mt-[30rpx]">
                        <u-button :text="t('verifyOther')" type="primary" shape="circle" @click="verifyDetail = null"
                            :plain="true"></u-button>
                    </view>
                </view>
                <view class="bg-white px-[30rpx] py-[50rpx] mx-[30rpx] rounded" v-else>
                    ...
                    <view class="my-[60rpx]">
                        <u-input :placeholder="t('inputPlaceholder')" border="surround" inputAlign="center"
                            v-model="verifyCode">
                            <!-- #ifdef MP -->
                            <template #suffix>
                    ...
                    <button
                        class="!bg-[var(--primary-color)] !text-[#fff] h-[80rpx] leading-[80rpx] rounded-[100rpx] text-[28rpx]"
                        :class="{ 'opacity-50': !verifyCode }" :disabled="!verifyCode" @click="search">{{ t('search')
                        }}</button>
                </view>
                <view class="text-center mt-[20rpx]">
                    <view @click=" redirect({ url: '/addon/vipcard/pages/verify/record' })">
