	...
	<view class="bg-[#F6F8FA] min-h-screen overflow-hidden" :style="themeColor()">
		<view v-if="!loading">
			<view class="bg-[#fff] rounded-lg mx-3 mt-4 p-3">
				<view class="flex">
					<image class="w-[240rpx] h-[180rpx] rounded mr-3" :src="img(detail.goods.cover_thumb_mid)"
						mode="aspectFill"></image>
					<view class="flex flex-col py-1 flex-1">
						<view class="font-bold multi-hidden">{{ detail.goods.goods_name }}</view>
						<view class="text-xs text-[var(--text-color-light6)]" v-if="detail.goods.keywords">
							{{ detail.goods.keywords }}</view>
			...
			<view class="flex flex-col mx-3 mt-4 px-3 py-4 bg-[#fff] rounded-lg">
				...
				<view class="text-xs text-[#888] mb-2">
					<text v-if="detail.card_type == 'commoncard' && detail.total_num">{{ t('hitCount') +
						detail.total_num }} | 已使用: {{ detail.total_use_num }} | 还剩 : {{ detail.total_num -
							detail.total_use_num }} </text>
				</view>
				<view view v-for="(item, index) in detail.member_card_item" :key="index"
					:class="['bg-[#FBF9FC] rounded-md flex p-2 items-center', { 'mb-3': detail.member_card_item.length - 1 != index }]">
					<image class="w-[160rpx] h-[160rpx] rounded mr-3" :src="img(item.cover_thumb_small)" mode="">
					</image>
					<view view class="flex-1 py-2 flex flex-col">
						<view class="multi-hidden font-bold"> {{ item.goods_name }}</view>

						<view class="text-xs text-[var(--text-color-light6)] mt-auto"
							v-if="detail.card_type == 'oncecard'">{{ t('usable') }}x{{ detail.total_num }}</view>
						<view class="text-xs text-[var(--text-color-light6)] mt-1"
							v-if="detail.card_type == 'oncecard'">
							{{ t('haveBeen') }}x{{ detail.total_use_num }}</view>
						<view class="text-xs text-[var(--text-color-light6)] mt-1"
							v-if="detail.card_type == 'oncecard'">
							还剩: {{ detail.total_num - detail.total_use_num }} 次</view>
					</view>
					<button
						...
						@click="verificationFn(item)">{{ t('verification') }}</button>
				</view>
				<view class="text-sm text-[#888] flex items-center self-center mt-4"
			...
			<u-popup :show="verificationShow" @close="verificationShow = false">
				...
				<view class="flex flex-col p-3 pb-5 items-center">
					<image :src="verificationData.verify_code_barcode" mode="widthFix"
						class="mb-5 w-[546rpx] border-1 border-[#aba9aa] border-dashed p-1"></image>
					<image :src="verificationData.verify_code_qrcode" mode="widthFix"
						class="w-[326rpx] border-1 border-[#aba9aa] border-dashed"></image>
