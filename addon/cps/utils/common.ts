...
import { getCpsLink } from '@/app/api/cps'
...
export const diyRedirect = (link: any) => {
	...
	if (link.url && (link.url.indexOf('https') != -1 || link.url.indexOf('http') != -1)) {
		if (link.url.indexOf('addon/cps/pages/cps') !== -1) {
				...
				if(res.data.title){
				    ...
				    if (res.data.web_url) {
				        	redirect({
		            			...
		            			param: { src: encodeURIComponent(res.data.web_url) }
		            		}); 
				    }
					// #endif 
				    // #ifdef MP-WEIXIN
				...
				if(res.data.type=='h5'){
					...
					// #ifdef H5
						if (res.data.url.h5) {
							redirect({
		            			url: '/app/pages/webview/index',
		            			param: { src: encodeURIComponent(res.data.url.h5) }
		            		}); 
						}
					// #endif 
