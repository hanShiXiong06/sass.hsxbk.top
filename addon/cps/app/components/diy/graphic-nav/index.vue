				...
				<view class="graphic-nav-item" :class="{'flex-shrink-0' : diyComponent.showStyle == 'singleSlide'}"
					...
					<app-link :data="item.link" custom-class="flex flex-col items-center box-border py-2">
						...
						<text v-if="diyComponent.mode != 'img'"
							...
							class="graphic-text w-full text-center truncate leading-normal"
							:class="{ 'pt-1.5' : diyComponent.mode != 'text' }"
							:style="{ fontSize: diyComponent.font.size * 2 + 'rpx', fontWeight: item.link.url == configStore.tabsIndex?'bold':diyComponent.font.weight, color: item.link.url == configStore.tabsIndex?'rgb(0, 186, 173)':diyComponent.font.color }">
							{{ item.title }} 
						</text>
					</app-link>
				</view>
...
<script lang="ts" setup>
	...
	const onClick = ({link: {url}}:{link: {url: number}})=> {
		if(!isNaN(url)) {
			configStore.tabsIndex = Number(url)
		}
	}
