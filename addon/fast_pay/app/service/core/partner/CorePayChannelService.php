...
class CorePayChannelService extends BaseCoreService
{
    ...
    public function getConfigByChannelAndType(int $site_id, string $channel, string $type){
        ...
        if(!$pay_channel->isEmpty()){
            if($type == 'fastpay_partner_wechatpay'){
                $pay_channel->config = array_merge($pay_channel->config, $this->getWechatPayFullConfig($site_id),$this->getAdminConfig());
            }
    ...
    public function getWechatPayFullConfig(int $site_id){
        ...
        return [
            'sub_mp_app_id' => $mp_app_id,
            'sub_mini_app_id' => $mini_app_id,
//            'mp_app_id' => $mp_app_id,
//            'mini_app_id' => $mp_app_id
            //............
        ];
    }
    public function getAdminConfig()
    {
        ...
        if (empty($info)) {
            throw new Exception('服务商基本配置未完成');
        }
        return $info['value'];
    }
}