...
use addon\fast_pay\app\service\core\businesspay\CorePayChannelService;
...
class CorePayEventService extends BaseCoreService
{
     ...
     * @return $this
     */
    public function init(int $site_id, string $channel = '', string $type = '',$business_id='')
    {
        $this->site_id = $site_id;
        $this->channel = $channel;
        $this->type = $type;
        $this->config = (new CorePayChannelService())->getConfigByChannelAndType($this->site_id, $this->channel, $this->type,$business_id);
        return $this;
    }
    ...
    {
        $notify_url = (string)url("/api/fast_pay/pay/partnernotify/$this->site_id/$this->channel/$this->type/$action", [], '', true);//异步回调通知地址
    ...
    public function pay(string $out_trade_no, float $money, string $body, string $refund_url = '', string $quit_url = '', string $buyer_id = '', string $openid = '', string $voucher = '')
    {
        ...
        switch ($this->type) {
            case 'fastpay_business_wechatpay':
    ...
    public function transfer(float $money, string $transfer_no, string $to_no, string $to_name, string $remark, array $transfer_list = [], string $to_type = '', string $product_code = '', string $scene = '')
    {
        ...
        switch ($transfer_type) {
            case 'fastpay_partner_wechatpay':
                $money *= 100;
                break;
            case PayDict::ALIPAY:
                break;
    ...
    {
        if ($this->type == 'fastpay_partner_wechatpay') {
            $money *= 100;
            $total *= 100;
        }
