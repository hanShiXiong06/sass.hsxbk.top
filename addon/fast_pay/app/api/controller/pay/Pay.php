...
namespace addon\fast_pay\app\api\controller\pay;

use addon\fast_pay\app\service\api\pay\BusinessPayService;
...
class Pay extends BaseApiController
{
    ...
    public function businessnotify($site_id, $channel, $type, $action)
    {
        return (new BusinessPayService())->notify($channel, $type, $action);
    }
    public function businesspay()
    {
        ...
        return success('SUCCESS',(new BusinessPayService())->pay($data['type'], $data['trade_type'], $data['trade_id'], $data['return_url'], $data['quit_url'], $data['buyer_id'], $data['voucher'], $data['openid'],$data['business_id']));
    }
