  ...
  >
    ...
    >
      <el-form-item :label="t('mchId')" prop="config.mch_id">
        <el-input
          v-model.trim="formData.config.mch_id"
      ...
      <el-form-item :label="t('mchSecretKey')" prop="config.mch_secret_key">
        <el-input
          v-model.trim="formData.config.mch_secret_key"
          :placeholder="t('mchSecretKeyPlaceholder')"
          class="input-width"
          maxlength="32"
          show-word-limit
          clearable
      ...
      <el-form-item :label="t('mchSecretCert')" prop="config.mch_secret_cert">
        <div class="input-width">
          <upload-file
            v-model="formData.config.mch_secret_cert"
      ...
      <el-form-item
        :label="t('mchPublicCertPath')"
        prop="config.mch_public_cert_path"
      >
        <div class="input-width">
          <upload-file
            v-model="formData.config.mch_public_cert_path"
            api="sys/document/wechat"
          />
        </div>
...
const formRules = computed(() => {
  return {
    ...
    "config.mch_secret_key": [
      {
        required: true,
        message: t("mchSecretKeyPlaceholder"),
        trigger: "blur",
      },
    ],
    "config.mch_secret_cert": [
      {
        required: true,
        message: t("mchSecretCertPlaceholder"),
        trigger: "blur",
      },
    ],
    "config.mch_public_cert_path": [
      {
        required: true,
        message: t("mchPublicCertPathPlaceholder"),
        trigger: "blur",
      },
    ],
  };
...
const enableVerify = () => {
  let verify = true;
  if (
    Test.empty(formData.config.mch_id) ||
    Test.empty(formData.config.mch_secret_key) ||
    Test.empty(formData.config.mch_secret_cert) ||
    Test.empty(formData.config.mch_public_cert_path)
  )
    verify = false;
  return verify;
};
