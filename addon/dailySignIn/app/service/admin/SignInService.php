...
class SignInService extends BaseAdminService{
    ...
    public function create($request){
        ...
        if(!empty($request['data'])){
            ...
            $response =  (new CoreConfigService())->setConfig(SignInLogs::SYS_CONFIG_KEY,[
                'cycle'         =>$request['cycle'],
                'isSupplement'  =>$request['isSupplement'],
                'adUnitId'      =>$request['isSupplement']?$request['adUnitId']:'',
                'data'          =>$request['data']
            ]);
            (new CoreConfigService())->modifyStatus(SignInLogs::SIGN_IN_OPEN,SignInLogs::SYS_CONFIG_KEY);
        }else{
            (new CoreConfigService())->modifyStatus(SignInLogs::SIGN_IN_CLOSE,SignInLogs::SYS_CONFIG_KEY);
        }
