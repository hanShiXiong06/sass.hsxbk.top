          ...
          <view class="signin-wrap">
            <view class="member-info">
              <view class="headimg">
                ...
                <view class="signin-info">
                  <view>
                    {{ signInData.isSupplement ? "已签到" : "已连续签到" }}
              ...
              <view class="signin-day-list">
                <view class="signin-day-con">
                  <view class="signin-day-scroll">
                    <block
                        v-for="item in signInData.showSignDays"
                        :key="item.day"
                    >
                      <view
                          v-if="!item.is_last"
                          ...
                          :class="{
                          signed: item.is_hasSign,
                          supplement:
                            item.is_hasSign == false && signInData.hasSign == 1,
                        }"
                          @click="
                          item.is_hasSign == false && handleSignClick(item)
                        "
                      >
                        ...
                        <image
                            :src="
                            img(
                              item.is_hasSign == false &&
                                signInData.hasSign == 1
                                ? '/addon/dailySignIn/signin/supplement.png'
                                : '/addon/dailySignIn/signin/sign-icon.png'
                            )
                          "
                        ></image>
                      ...
                      <view
                          ...
                          :class="{
                          signed: item.is_hasSign,
                          supplement: item.is_hasSign == false,
                        }"
                          @click="
                          item.is_hasSign == false && handleSignClick(item)
                        "
                        ...
                        <image
                            ...
                            img(
                              item.is_hasSign == false &&
                                signInData.hasSign == 1
                                ? '/addon/dailySignIn/signin/supplement.png'
                                : '/addon/dailySignIn/signin/sign-box.png'
                            )
                          "
                            mode="widthFix"
                        />
            ...
            <view class="signin-rule">
              ...
              <view class="signin-rule-con">
                ...
                <view class="rule-item" v-if="signInData.isSupplement">
                  {{ signInData.rule.length + 1 }}.
                  补签是指用户昨日遗忘签到了，可以到次日当天签到成功了，可以补签未签到的天数，获得相应的奖励
                </view>
                <view class="rule-item">
                  {{ signInData.rule.length + 2 }}.连续签到{{
                    signInData.cycle
                  }}天为一个周期，连续签到天数签满一个周期或者签到中断，将清空连签天数重新计算签到天数
                </view>
                <view class="rule-item">
                  {{ signInData.rule.length + 3 }}.
...
const checkSignInStatus = () => {
  checkSignInStatusApi().then((res) => {
    if (res.code == 1) {
      ...
      signInData.value.isSignList = res.data.isSignList;
...
const getRule = () => {
  ...
  getSignInAwardRules().then((res) => {
    if (res.code == 1) {
      ...
      signInData.value.isSupplement = res.data.isSupplement;
      signInData.value.adUnitId = res.data.adUnitId;
      ...
      for (let i = 1; i <= res.data.cycle; i++) {
        if (i >= start_day && i <= end_day) {
          showSignDays.push({
            day: i,
            is_last: 0,
            is_hasSign: signInData.value.isSignList[i - 1],
            point: default_point,
          });
        }
      }
      ...
      for (let i in showSignDays) {
        ...
        signInData.value.isContinuous =
            showSignDays[i]["is_hasSign"] === true ? 1 : 0;
      }
...
const handleSignClick = (item) => {
  uni.showModal({
    title: "温馨提示",
    content: "抱歉！该功要在观看完视频后方可进行签到，感谢您的支持。",
    confirmColor: "#5ac725",
    showCancel: true,
    success: (res) => {
      if (res.confirm) {
        ...
        if (wx.createRewardedVideoAd) {
          videoAd = wx.createRewardedVideoAd({
            adUnitId: signInData.value.adUnitId,
          });
          videoAd.onLoad(() => {});
          if (!signInData.value.isEnded) {
            // 用户触发广告后，显示激励视频广告
            if (videoAd) {
              videoAd.show().catch(() => {
                videoAd
            ...
            if (res && res.isEnded) {
              supplementApi({
                ...
                if (res.code == 1) {
                  ...
                  getSignGrowthData();
                  signInData.value.hasSign = 1;
                  signInData.value.signDaysSeries =
                      signInData.value.signDaysSeries + 1;
                }
              });
            } else {
              uni.showToast({
