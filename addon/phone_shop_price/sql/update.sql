...
CREATE TABLE `{{prefix}}phone_shop_recycle_order` (
    ...
    `express_id` varchar(50) NOT NULL DEFAULT '' COMMENT '快递单号',
    `send_username` varchar(255) NOT NULL COMMENT '寄件人',
    `telphone` char(11) NOT NULL COMMENT '手机号',
    `pay_type` enum('微信','支付宝','银行卡') DEFAULT NULL COMMENT '收款方式',
    `account` varchar(255) NOT NULL COMMENT '收款账号',
    `delivery_type` varchar(20) NOT NULL DEFAULT 'mail' COMMENT '发货方式：mail-邮寄，self-自送',
    `return_type` varchar(20) NOT NULL DEFAULT 'self' COMMENT '退货方式：self-自取，mail-邮寄',
    `qrcode_image` varchar(255) DEFAULT '' COMMENT '收款码图片',
    `return_address` varchar(255) DEFAULT '' COMMENT '退货地址',
    `status` varchar(20) DEFAULT '1' COMMENT '订单状态:1待签收,2待质检,3质检中,4质检完成,5订单完成,6有退货,7全部退货',
    ...
    `close_express_id` varchar(50) NOT NULL DEFAULT '' COMMENT '退货单号',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_order_no` (`order_no`),
    KEY `idx_member_site` (`member_id`,`site_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci COMMENT='回收订单主表';

-- 创建回收订单设备表
CREATE TABLE `{{prefix}}phone_shop_recycle_order_device` (
    `id` int NOT NULL AUTO_INCREMENT,
    `order_id` int NOT NULL COMMENT '订单ID',
    `imei` varchar(50) NOT NULL COMMENT 'IMEI号码',
    `model` varchar(255) NOT NULL DEFAULT '待识别' COMMENT '设备型号',
