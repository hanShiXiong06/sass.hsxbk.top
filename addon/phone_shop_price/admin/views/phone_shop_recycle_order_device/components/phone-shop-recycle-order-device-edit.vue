<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updatePhoneShopRecycleOrderDevice') : t('addPhoneShopRecycleOrderDevice')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
...
import { addPhoneShopRecycleOrderDevice, editPhoneShopRecycleOrderDevice, getPhoneShopRecycleOrderDeviceInfo } from '@/addon/phone_shop_price/api/phone_shop_recycle_order_device'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editPhoneShopRecycleOrderDevice : addPhoneShopRecycleOrderDevice
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getPhoneShopRecycleOrderDeviceInfo(row.id)).data
