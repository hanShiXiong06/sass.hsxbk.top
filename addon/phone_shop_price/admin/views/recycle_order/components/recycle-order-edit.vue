<template>
    <el-dialog v-model="showDialog" :title="formData.id ? t('updateRecycleOrder') : t('addRecycleOrder')" width="50%" class="diy-dialog-wrap" :destroy-on-close="true">
...
import { addRecycleOrder, editRecycleOrder, getRecycleOrderInfo } from '@/addon/phone_shop_price/api/recycle_order'
...
const confirm = async (formEl: FormInstance | undefined) => {
    ...
    let save = formData.id ? editRecycleOrder : addRecycleOrder
...
const setFormData = async (row: any = null) => {
    ...
    loading.value = true
    if(row){
        const data = await (await getRecycleOrderInfo(row.id)).data
