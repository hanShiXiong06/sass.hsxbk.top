...
CREATE TABLE `{{prefix}}shop_giftcard` (
  ...
  `material_ids` text NOT NULL COMMENT '礼品卡封面id，多个逗号隔开',
...
CREATE TABLE `{{prefix}}shop_giftcard_give` (
  ...
  `member_card_id` int NOT NULL DEFAULT '0' COMMENT '赠予的卡id',
  `to_member_id` int NOT NULL DEFAULT '0' COMMENT '要给予的会员id',
  `status` varchar(50) NOT NULL DEFAULT '' COMMENT '状态',
  `blessing` text NOT NULL COMMENT '祝福语',
  `create_time` int NOT NULL DEFAULT '0' COMMENT '创建时间',
  `get_time` int NOT NULL DEFAULT '0' COMMENT '领取时间',
  PRIMARY KEY (`give_id`)
...
CREATE TABLE `{{prefix}}shop_giftcard_make` (
  ...
  `status` varchar(255) NOT NULL COMMENT '状态，wait：未开始，proceed：进行中，finish：完成，fail：失败',
