...
/**
 * 兑换卡导出数据源查询
 * Class GoodsCardExportDataListener
...
class GoodsCardExportDataListener
{
    ...
    {
        $data = [];
        if ($param[ 'type' ] == 'shop_giftcard_goods_card') {
            $card_model = new Card();
            $order = 'card_no asc';
            //查询导出卡密数据
            $search_model = $card_model->where([ [ 'site_id', '=', $param[ 'site_id' ] ] ])->withSearch([ 'giftcard_id', 'card_make_id' ], $param[ 'where' ])->field('card_no,card_key')->order($order);
