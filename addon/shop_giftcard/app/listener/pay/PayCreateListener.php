...
class PayCreateListener
{
    public function handle(array $params)
    {
        ...
        if ($trade_type == OrderDict::TYPE) {
            $order_info = ( new Order() )->where([ 'order_id' => $params[ 'trade_id' ] ])->findOrEmpty()->toArray();
            if ($order_info[ 'status' ] != OrderDict::WAIT_PAY) throw new CommonException('GIFTCARD_ONLY_WAIT_PAY_CAN_BE_PAY');
            //添加订单支付表
            return [
