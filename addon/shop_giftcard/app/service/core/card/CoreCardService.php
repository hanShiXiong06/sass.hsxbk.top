...
class CoreCardService extends BaseCoreService
{
    ...
    public function addVirtualCard($params)
    {
        ...
        $giftcard_info = $giftcard_model->field('giftcard_id, card_right_type, card_goods_type, card_goods_count, validity_type, validity_day, validity_time')
            ->where([
            ...
            try {
                ...
                for ($row = $init_row; $row <= $limit; ++$row) {
                    ...
                    try {
                        ...
                        if (strlen(trim($item_data[ 0 ])) != $giftcard_info[ 'card_no_length' ]) throw new CommonException('GIFTCARD_CARD_NO_LENGTH_NOT_MATCH');

                        if (strlen(trim($item_data[ 1 ])) != $giftcard_info[ 'card_key_length' ]) throw new CommonException('GIFTCARD_KEY_LENGTH_NOT_MATCH');

                        // 检测卡号是否存在
                        $card_no_count = $this->model->where([ [ 'site_id', '=', $params[ 'site_id' ] ], [ 'card_no', "=", $item_data[ 0 ] ] ])->count();
    ...
    public function expire($ids)
    {
        ...
        $field = 'card_id,giftcard_id,member_id,site_id';
