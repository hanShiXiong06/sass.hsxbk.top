...
class MemberRecordsService extends BaseApiService
{
    ...
    public function getGivePage(array $where = [])
    {
        ...
        foreach ($list[ 'data' ] as $k => $v) {
            // 查询卡信息
            $field = 'card_id, giftcard_id, card_cover,balance';
    ...
    public function getGiveDetail($member_card_id)
    {
        ...
        $card_model = new Card();
        // 查询卡信息
        $field = 'card_id, giftcard_id, card_cover';
    ...
    public function getReceivePages(array $where = [])
    {
        ...
        foreach ($list[ 'data' ] as $k => $v) {
            // 查询卡的领取记录
            $field = 'member_card_id, card_id, from_member_id, get_time';
            $list[ 'data' ][ $k ][ 'record_info' ] = $this->model->field($field)->where([
                [ 'site_id', "=", $this->site_id ],
                [ 'member_id', '=', $this->member_id ],
                [ 'card_id', '=', $v[ 'card_id' ] ],
                [ 'source', '=', CardDict::GIVE ]
            ])->with([ 'fromMember' ])->findOrEmpty()->toArray();
