		...
		<view class="give-info-bg w-full h-[100vh]" v-if="Object.keys(detail).length">
			<view class="pt-[183rpx]">
				<view class="rounded-[var(--rounded-big)] bg-[#fff] mx-[var(--sidebar-m)] px-[82rpx] pb-[140rpx]">
					...
					<view class="text-center text-[28rpx] font-500 leading-[39rpx] text-[var(--text-color-light6)] mt-[10rpx] truncate">{{'送给他的'+detail.card_info.giftcard.card_name}}</view>
					...
					<view class="text-[28rpx] mx-auto mt-[40rpx] multi-hidden" v-if="detail.give.blessing">
						<text class="text-[var(--text-color-light6)] leading-[39rpx]">您祝对方：</text>
						<text class="text-[#333] leading-[39rpx]">{{detail.give.blessing}}</text>
					</view>
					<view class="mt-[148rpx]">
						<button class="w-full h-[80rpx] font-500 text-[26rpx] leading-[80rpx] !text-[var(--text-color-light9)] !bg-[#F7F7F7] m-0 rounded-[var(--rounded-big)] remove-border">对方已领取</button>
					</view>
				</view>
			</view>
		</view>
