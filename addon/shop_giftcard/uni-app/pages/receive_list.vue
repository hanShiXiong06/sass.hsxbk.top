		...
		<view class="fixed left-0 top-0 right-0 z-10" v-if="statusLoading">
			<scroll-view :scroll-x="true" class="tab-style-2">
				<view class="tab-content">
					<view class="tab-items" :class="{ 'class-select': status === '' }" @click="statusFn('')">全部</view>
				...
				class="h-[430rpx] mb-[var(--top-m)] rounded-[var(--rounded-big)] overflow-hidden box-border relative">
					...
					<view class="flex flex-col justify-between w-full h-[430rpx] mb-[var(--top-m)] rounded-[var(--rounded-big)] overflow-hidden box-border absolute left-0 top-0 z-5">
						...
						<view class="flex items-center justify-between bg-[rgba(255,255,255,0.9)] h-[80rpx] px-[var(--pad-sidebar-m)] box-border">
							...
							<view class="flex items-center">
								<button v-if="item.status=='to_use' && item.giftcard.is_give" class="h-[50rpx] font-500 text-[22rpx] leading-[46rpx] !text-[#333] !bg-transparent m-0 rounded-[25rpx] px-[22rpx] border-[2rpx] border-solid border-[var(--text-color-light9)] box-border ml-[20rpx]" @click.stop="btnClick('give',item.card_id)">赠送好友</button>
								<button v-if="item.status=='to_use' || item.status=='can_use'" class="h-[50rpx] font-500 text-[22rpx] leading-[46rpx] !text-[var(--primary-color)] m-0 rounded-[25rpx] px-[22rpx] border-[2rpx] border-solid border-[var(--primary-color)] box-border ml-[20rpx]" @click.stop="btnClick('use',item.card_id)">去使用</button>
								<button v-if="item.status=='used'" class="h-[50rpx] font-500 text-[22rpx] leading-[50rpx] !text-[var(--text-color-light6)] !bg-[rgba(221,221,221,0.8)] m-0 rounded-[25rpx] px-[22rpx] box-border ml-[20rpx]" @click.stop="btnClick('use',item.card_id)">已使用</button>
								<button v-if="item.status=='invalid'" class="h-[50rpx] font-500 text-[22rpx] leading-[50rpx] !text-[var(--text-color-light6)] !bg-[rgba(221,221,221,0.8)] m-0 rounded-[25rpx] px-[22rpx] box-border ml-[20rpx]" @click.stop="btnClick('use',item.card_id)">已失效</button>
							</view>
						</view>
					</view>
				</view>
			</view>
			<mescroll-empty v-if="!list.length && !loading" :option="{tip : '暂无卡记录', icon: img('addon/shop_giftcard/empty.png')}"></mescroll-empty>
		</mescroll-body>
