		...
		<view v-if="Object.keys(detail).length" class="give-info-bg w-full h-[100vh]">
			<view class="pt-[183rpx]">
				<view class="rounded-[var(--rounded-big)] bg-[#fff] mx-[var(--sidebar-m)] px-[82rpx] pb-[140rpx]">
					...
					<view class="text-center text-[28rpx] font-500 leading-[39rpx] text-[var(--text-color-light6)] mt-[10rpx] truncate">{{'赠送给您' + detail.card_info.giftcard.card_name}}</view>
					...
					<view class="text-[28rpx] mx-auto mt-[40rpx] multi-hidden" v-if="detail.blessing">
						<text class="text-[var(--text-color-light6)] leading-[39rpx] mb-[10rpx]">对方祝您：</text>
						<text class="text-[#333] leading-[39rpx]">{{detail.blessing}}</text>
					</view>
					<view class="mt-[148rpx]">
						<button v-if="detail.status =='wait_give'"
						class="w-full h-[80rpx] font-500 text-[26rpx] leading-[80rpx] !text-[#fff] !bg-[var(--primary-color)] m-0 rounded-[var(--rounded-big)] remove-border"
						:class="{'opacity-40':disable}"
						@click="save">立即领取</button>
						<block v-else>
							<button v-if="detail && detail.to_have == 'my'" class="w-full h-[80rpx] font-500 text-[26rpx] leading-[80rpx] !text-[var(--text-color-light9)] !bg-[#F7F7F7] m-0 rounded-[var(--rounded-big)] remove-border">已领取</button>
							<button v-else class="w-full h-[80rpx] font-500 text-[26rpx] leading-[80rpx] !text-[var(--text-color-light9)] !bg-[#F7F7F7] m-0 rounded-[var(--rounded-big)] remove-border">该卡已被领取</button>
							<view class="mt-[25rpx] text-[24rpx] text-center leading-[34rpx] !text-[var(--text-color-light9)] " @click="tolink">查看卡包</view>
						</block>
					</view>
				</view> 
			</view>
			<uni-popup ref="popupRef" type="center" :is-mask-click="false">
				...
				:style="{'background-image':'url('+img('addon/shop_giftcard/popup_bg.png')+')',backgroundSize: 'cover',backgroundRepeat: 'no-repeat',backgroundPosition: 'top',}">
					...
					<view class="text-[40rpx] leading-[56rpx] font-bold text-center pt-[174rpx]">恭喜您 领取成功！</view>
					<view class="text-[24rpx] leading-[34rpx] text-center mt-[30rpx]">您已成功领取了1张礼品卡~</view>
...
<script setup lang="ts">
	...
	const toDetail = () =>{
		if(detail.value && detail.value.to_have == 'my'){
			redirect({ url: '/addon/shop_giftcard/pages/use_card', param:{card_id: detail.value.card_id}, mode: 'redirectTo' });
		}
	}
