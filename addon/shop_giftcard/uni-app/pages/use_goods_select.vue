	...
	<view class="bg-[#f8f8f8] min-h-[100vh]" :style="themeColor()">
		<template v-if="Object.keys(detail).length&&Object.keys(sku_data).length">
			<view v-if="tips" class="h-[66rpx] bg-[#FCF2DF] flex items-center px-[var(--sidebar-m)]">
				...
				<text class="text-[24rpx] leading-[34rpx] ml-[16rpx] font-400">以下商品存在已售罄或已下架的商品，请耐心等待卖家备货</text>
			</view>
				...
				<view class="card-template">
					...
					<view class="flex items-center justify-between pb-[20rpx] border-0 !border-b-[2rpx] border-solid border-[#f8f8f8]">
						<template v-if="detail.giftcard.card_goods_type=='diy'">
							<text class="title !mb-0">请在以下商品中任选{{detail.total_num - detail.use_num}}件商品</text>
						...
						<template v-else>
							<text class="title !mb-0">请兑换以下商品</text>
						</template>
					</view>
					...
					<block v-for="(item,index) in detail.cardGoods" :key='item.sku_id'>
						<view class="mt-[var(--pad-top-m)] flex">
							<view class="w-[210rpx] h-[210rpx] rounded-[var(--rounded-mid)] relative">
								<image v-if="item.sku_image" class="w-[210rpx] h-[210rpx] rounded-[var(--rounded-mid)]" :src="img(item.sku_image)" 
								@error="item.sku_image='static/resource/images/diy/shop_default.jpg'" :mode="'aspectFill'"/>
								...
								<view v-if="!item.stock" class="absolute w-full w-[210rpx] h-[210rpx] bg-[rgba(51,51,51,0.6)] z-10 left-0 top-0 rounded-[var(--rounded-mid)]">
									<view class="text-[#fff] leading-[210rpx] text-center text-[24rpx] font-500">该商品已售罄</view>
								</view>
								<view v-else-if="!item.status" class="absolute w-full w-[210rpx] h-[210rpx] bg-[rgba(51,51,51,0.6)] z-10 left-0 top-0 rounded-[var(--rounded-mid)]">
									<view class="text-[#fff] leading-[210rpx] text-center text-[24rpx] font-500">该商品已下架</view>
								</view>
							</view>
							<view class="ml-[20rpx] flex-1 flex flex-col justify-between">
								<view class="w-full">
									...
									<view v-if="detail.giftcard.card_goods_type == 'all'" class="mt-[20rpx] text-[24rpx] text-[var(--text-color-light6)] font-400 truncate leading-[34rpx] max-w-[432rpx]">最多可选{{item.total_num}}件 已兑换{{item.use_num}}件</view>
								</view>
								<view class="flex items-center justify-between h-[52rpx]">
									...
									<view v-if="sku_data[item.sku_id].max_num" class="flex items-center h-[52rpx]">
										<text @click.stop="skuNumChange(item.sku_id,'jian')" class="text-[24rpx] font-500 nc-iconfont nc-icon-jianV6xx leading-[52rpx]"
											:class="{'text-[var(--text-color-light9)]':!sku_data[item.sku_id].num }"></text>
										<input class="text-[#303133] text-[28rpx] mx-[10rpx] w-[80rpx] h-[44rpx] bg-[var(--temp-bg)] leading-[44rpx] text-center rounded-[6rpx]" type="number" @blur="skuNumChange(item.sku_id,'')" v-model="sku_data[item.sku_id].num"  />
										<text @click.stop="skuNumChange(item.sku_id,'jia')" class="text-[24rpx] font-500 nc-iconfont nc-icon-jiahaoV6xx leading-[52rpx]"
			...
			<view class="border-[0] border-t-[2rpx] border-solid border-[#f5f5f5] w-[100%] flex justify-between pl-[30rpx] pr-[20rpx] bg-[#fff] box-border fixed left-0 bottom-0 tab-bar z-1 items-center">
				...
				<view class="flex flex-1 items-center">
					<button
					 v-if="detail.status=='to_use' || detail.status=='can_use'"
					class="w-full !h-[70rpx] font-500 text-[26rpx] !text-[#fff] primary-btn-bg !m-0 leading-[70rpx] rounded-full remove-border"
					:class="{'opacity-40':disable}" @click="save">确认兑换</button>
					<button
					 v-if="detail.status=='used'"
					class="w-full !h-[70rpx] font-500 text-[26rpx]  !text-[#fff] !bg-[var(--text-color-light9)] !m-0 leading-[70rpx] rounded-full remove-border"
					>已使用</button>
					<button
					 v-if="detail.status=='invalid'"
					class="w-full !h-[70rpx] font-500 text-[26rpx] !text-[#fff] !bg-[var(--text-color-light9)] !m-0 leading-[70rpx] rounded-full remove-border"
					>已失效</button>
				</view>
			</view>
...
<script setup lang="ts">
	...
	const save = ()=> {
		...
		if (!use_num.value) {
			uni.showToast({ title: '请选择需要兑换的商品', icon: 'none' })
			return
		}
		if (detail.value.giftcard.card_goods_type == 'diy' && detail.value.giftcard.delivery_way == 'all') {
			if (use_num.value < (detail.value.total_num - detail.value.use_num)) {
				uni.showToast({ title: '该礼品卡必须全部提货', icon: 'none' })
				return
			}
		}
		disable.value = true
