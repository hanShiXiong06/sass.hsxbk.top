				...
				<div ref="imageBoxRef">
					<div v-for="(item,index) in diyStore.editComponent.list" :key="item.id" class="item-wrap p-[10px] pb-0 relative border border-dashed border-gray-300 mb-[16px]">
						...
						<el-form-item label="点击类型">
							<el-radio-group v-model="item.showType" class="mb-[18px]">
								<el-radio label="popup">弹出</el-radio>
						...
						<div v-if="item.showType == 'popup'">
							<el-form-item label="标题">
								<el-input v-model="item.text" placeholder="标题" clearable show-word-limit/>
							</el-form-item>
							<el-form-item label="弹出图片">
								<upload-image v-model="item.imgPath" :limit="1"/>
							</el-form-item>
						</div>
					</div>
				</div>
