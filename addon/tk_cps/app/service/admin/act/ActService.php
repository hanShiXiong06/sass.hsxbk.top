...
class ActService extends BaseAdminService
{
    ...
    /**
     * @Notes:获取驱动列表
     * @Interface getDrivers
     * @return array
     * @throws \think\Exception
     * @author: TK
     * @Time: 2024/10/7   下午7:01
     */
    public function getDrivers()
    {
        $data=(new CommonService())->getSiteDriver($this->site_id);
        $list=[];
        foreach ($data as $k => $v){
            $list[]=[
                'name'=>$v['name'],
    ...
    {
        $field = 'id,site_id,act_id,act_name,type,desc,img,icon,poster,commission_rate,introduce,attribution_explain,settlement_time,start_date,end_date,create_time';
    ...
    public function shareInfo(int $id)
    {
        ...
        $data = (new V2CpsService())->getActInfo($info['act_id'], $info['type']);
        return $data;
    }
