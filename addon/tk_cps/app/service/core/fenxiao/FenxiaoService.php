...
namespace addon\tk_cps\app\service\core\fenxiao;
    ...
    public function fenxiaoEvent($orderInfo)
    {
        ...
        if ($p_member_info['member_level'] && !empty($p_member_info['member_level']['level_benefits'])) {
            ...
            foreach ($level_benefits as $k => $v) {
                if ($k == 'tk_cps_fenxiao' && $v['is_use'] == 1) {
                    $commission=0;
                    //比列
                    ...
                    (new CoreMemberAccountService())->addLog($orderInfo['site_id'], $fenxiaoMemberInfo['pid'], MemberAccountTypeDict::COMMISSION, $commission, 'cps_award', 'CPS活动一级分销激励');
        ...
        if ($pp_member_info['member_level'] && !empty($pp_member_info['member_level']['level_benefits'])) {
            ...
            foreach ($level_benefits as $k => $v) {
                if ($k == 'tk_cps_fenxiao' && $v['is_use'] == 1) {
                    ...
                    (new CoreMemberAccountService())->addLog($orderInfo['site_id'], $pp_fenxiaoMemberInfo['pid'], MemberAccountTypeDict::COMMISSION, $commission, 'cpd_award', 'CPS活动二级分销激励');
                    $fenxiaoOrderModel->where(['order_id' => $orderInfo['order_id']])->update([
                        'status' => 1,
                        'update_time' => time()
                    ]);
