...
class BwcService extends BaseApiService
{
    ...
    public function actInfo($data)
    {
        ...
        if(isset($res['data']['actionUrl']['mtMonitor'])){
            if($res['data']['source']==1){
                $imp='https:'.$res['data']['actionUrl']['mtMonitor']['impMonitorUrl'];
                $click='https:'.$res['data']['actionUrl']['mtMonitor']['clickMonitorUrl'];
                file_get_contents($imp);
                file_get_contents($click);
            }
        }
        return $res;
    }
    ...
    /**
     * @Notes:新获取活动商家接口
     * @Interface newActList
     * @param $data
     * @return array|mixed
     * @throws Exception
     * @author: TK
     * @Time: 2024/10/29   上午8:29
     */
    public function newActList($data)
    {
        $config = $this->getSiteConfig($this->site_id);
        $rate = $config['fanxianratio'] / 100;
        $res=$this->execute('bwc/union/new/merchantList', $data);
        $merchantList=$res['data']['merchantList'];
        foreach ($merchantList as $k => $v) {
            if($v['planList']){
                foreach ($v['planList'] as $k1=>$v1){
