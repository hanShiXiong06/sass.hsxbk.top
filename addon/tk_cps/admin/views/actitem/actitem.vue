<template>
    <div class="main-container">
        <el-card class="box-card !border-none" shadow="never">
            <el-alert type="warning" title="活动推广详情信息，如遇到推广链接失效，可以删除推广或者在活动列表里面点击立即推广，推广信息将在用户访问或者操作时候更新" :closable="false"
                show-icon />
            <el-card class="box-card !border-none my-[10px] table-search-wrap" shadow="never">
                <el-form :inline="true" :model="actItemTable.searchParam" ref="searchFormRef">
                    <el-form-item :label="t('actName')" prop="act_name">
                        <el-input v-model="actItemTable.searchParam.act_name" :placeholder="t('actNamePlaceholder')" />
                    </el-form-item>
                    <el-form-item label="类型" prop="type">
                        <el-select class="w-[280px]" v-model="actItemTable.searchParam.type" clearable
                    ...
                    <el-form-item>
                        <el-button type="primary" @click="loadActItemList()">{{ t('search') }}</el-button>
            ...
            <div class="mt-[10px]">
                <el-table @selection-change="handleSelectionChange" :data="actItemTable.data" size="large"
                    v-loading="actItemTable.loading">
                    <template #empty>
                        <span>{{ !actItemTable.loading ? t('emptyData') : '' }}</span>
                    </template>
                    <el-table-column type="selection" width="55" />
                    <el-table-column prop="act_name" :label="t('actName')" min-width="180" :show-overflow-tooltip="true" />

                    <el-table-column prop="type" :label="t('type')" min-width="40" :show-overflow-tooltip="true">
                    ...
                    <el-table-column label="支持渠道" min-width="120">
                        <template #default="{ row }">
                            ...
                            <el-tag class="ml-2" v-if="JSON.parse(row.weapp).appid != ''">微信小程序</el-tag>
                            <el-tag class="ml-2" v-if="JSON.parse(row.aliapp).appid != ''">支付宝小程序</el-tag>
                        </template>
                    </el-table-column>
                    <el-table-column :label="t('operation')" fixed="right" min-width="120">
                        <template #default="{ row }">
                            <el-button type="primary" link @click="editEvent(row)">推广信息</el-button>
                ...
                <div class="mt-[16px] flex justify-between">
                    ...
                    <el-pagination v-model:current-page="actItemTable.page" v-model:page-size="actItemTable.limit"
                        layout="total, sizes, prev, pager, next, jumper" :total="actItemTable.total"
                        @size-change="loadActItemList()" @current-change="loadActItemList" />
                </div>
            </div>
...
import { getActItemList, deleteActItem, delselect } from '@/addon/tk_cps/api/actitem'
