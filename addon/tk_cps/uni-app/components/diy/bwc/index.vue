		...
		<view v-if="diyComponent.showtitle == 1" class="flex items-center mb-[16rpx] mt-[32rpx] justify-between mr-2">
          ...
          fontSize: diyComponent.titlesize + 'rpx',
			...
			<view class="flex items-center">
				...
				<view @click="changeAddress()" class="text-[#ffab45] text-xs tk-sltext" :style="{
            color: diyComponent.localcolor,
            fontSize: diyComponent.localsize + 'rpx',
          }">
					{{ currentPosition == "" ? "选择位置" : currentPosition }}
				</view>
				<view>
	...
	const getAddressByLatlngFn = () => {
		...
		getAddressByLatlng(data).then((res : any) => {
			if (res.data && Object.keys(res.data).length) {
				...
				uni.setStorageSync("currentPositiontk", currentPosition.value);
			} else {
