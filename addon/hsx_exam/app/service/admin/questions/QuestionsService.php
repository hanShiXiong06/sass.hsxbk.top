...
use addon\hsx_exam\app\service\admin\questions\CategoryService;
 ...
 */
class QuestionsService extends BaseAdminService
{
...
//        }
    ...
    {
        $field = ' category_id, id, questions_type, questions_desc , answer , options ';
        // 默认清空下 不需要分类 全部显示
        // 默认搜索条件
        $sku_where = ["questions_desc"];

        // 如果提供了 category_id，添加到搜索条件中
        if (!empty($where['category_id'])) {
            $sku_where[] = "category_id";
        }

        // 如果提供了 questions_type，添加到搜索条件中
        if (!empty($where['questions_type'])) {
            $sku_where[] = "questions_type";
        }

        $search_model = $this->model->where([ [ 'site_id', '=', $this->site_id ] ])->withSearch($sku_where, $where)
            ->field($field);
    ...
    public function del($ids)
    {
        ...
        $res = $this->model::destroy(function($query) use ($ids) {
            $query->where([ [ 'id', 'in', $ids ], [ 'site_id', '=', $this->site_id ] ]);
        });
        return $res;
    }
