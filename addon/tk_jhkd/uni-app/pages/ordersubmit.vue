<template>
	<view class="page">
		...
		<view class="p-2 icon_deration shadow  rounded-lg  ml-2 mr-2   bg-[#fff] ">


			<view class="fb flex item-center ">
				<view class="fb flex item-center ">
					<view class="mr-2 mt-3">
						<text class="tag" style="background: #000; color: #ffffff">寄 </text>
					</view>
					<view class="w-[380rpx]">
						<text v-if="form.startAddress == []" class="font-bold">从哪里寄？</text>
						<text v-if="form.startAddress == []" class="tk-sltext">请选择寄件地址</text>

						<text v-if="form.startAddress" class="font-bold tk-sltext text-[#000] text-sm">
				...
				<view class="address_box">
					<text class="text-[#000] " @click="toSelectAddress('startaddress')">地址簿</text>
				</view>
			</view>
			<view class="icon_tihuan">
				<i @click="exchangeAddress" class="nc-iconfont nc-icon-shangxiaxuanzeV6xx" data-v-2fd641c1=""
					style="color: var(--color); font-size: 24px;"></i>
			</view>
			<view class="line-box1 mt-2 ml-10"></view>
			<view class="fb fb flex item-center mt-2">
				<view class="fb flex item-center ">

					<view class="mr-2 mt-3">
						<text class="tag " style="background: #f00; color: #ffffff">收</text>
					</view>
					<view class="w-[380rpx]">
						<view v-if="form.endAddress == []" class="font-bold">收件人信息？</view>
						<text v-if="form.endAddress == []" class="tk-sltext">请选择收件地址</text>
						<text v-if="form.endAddress" class="font-bold text-[#000] tk-sltext text-sm">
				...
				<view class=" address_box">
					<view class="text-[#000] " @click="toSelectAddress('endaddress')">地址簿</view>
				</view>
			</view>

			<view class="line-box1 mt-2"></view>
			<view class="text-sm mt-2 text-[#686868] font-bold" style="font-size:22rpx">
				请认真核实地址,填写实际重量再下单！
			</view>
		</view>
		<view class="tk-card">
		...
		<view v-if="preData" class="mt-[20rpx] overflow-y-auto  bg-[#fff] m-2 rounded">
			<block v-for="(item, index) in preData" :key="index">
				<view class="fb p-[12rpx] tk-card1 icon_deration" :class="[

					{ 'selectitemclass': currentIndex == index },
				]" @click="selectPre(index)" v-if="item.onlinePay == 'Y'">
					<view class="fl">
						<view>
							<image style="width: 68rpx; height: 68rpx; border-radius: 8rpx" :src="img(item.logo)"
								mode="">
							</image>
						</view>
						...
						<view class="ml-[18rpx] " style="width:100% !import">
							<view class="flex items-center justify-between w-[580rpx]">
								<view class="flex items-center">
									...
									<view class="text-xs text-[#272822] ml-4  w-[160rpx]">限重:{{ item.limitWeight
										}}kg
									</view>
								</view>
							...
							<view v-if="item.price_rule.add" class="flex text-xs  mt-1 font-bold ">
								...
								<view hover-class="none" class="duihaoV6xx" v-if="currentIndex == index">
									<i class="nc-iconfont nc-icon-duihaoV6xx-1 " data-v-2fd641c1=""
										style="color: var(--color); font-size:48px;"></i>
								</view>
		...
		<view class="b-tabbar safe-area-inset-bottom bg-[#fff] shadow">
			<view class="" v-if="jhkdservice">
				...
				<view class=" fb items-center pl-2 pr-2 pb-2 pt-2">
					<view class="">
						<view class="flex">
							...
							<view class="text-xs font-bold tk-sltext">{{ selectData ? selectData.channelName :
								"请选择快递"
								}}
							</view>
						</view>
...
const exchangeAddress = () => {
	// 寄件地址
	const startAddress = form.startAddress;
	// 收件地址
	const endAddress = form.endAddress;

	form.startAddress = endAddress;
	form.endAddress = startAddress;
...
.page {
	background-color: rgba(236, 230, 230, 0.688);
	padding-top: 15rpx;
}

:root,
.tk-card1 {
	background-color: rgba(213, 225, 226, 0.3);
	margin: 24rpx;
	border-radius: 12rpx;
	padding: 12rpx;
