<template>
	<view class="bg-[#F6F6F6] min-h-[100vh] w-full" :style="themeColor()" v-if="memberStore.info">
		<view class="reseller fixed w-full z-2 !bg-[#F6F6F6]">
			<view class="top mt-[-44rpx]"
				:style="{ ...headerStyle, backgroundImage: 'url(' + img('addon/business_agent/fenxiao/bjfx.png') + ')' }">
				...
				<view class="top1">
					...
					<view class="fx" @click="shareEvent()">
						<image class="icon1" :src="img('addon/business_agent/fenxiao/fenxiang.png')" mode="aspectFit">
						</image>
			...
			<view v-if="fenxiaoinfo" class="icon">
				<view class="ico">
					<view class="tubiao">
						<image class="img" :src="img('addon/business_agent/fenxiao/yifx.png')"></image>
					</view>
					<view class="tt">
				...
				<view class="ico">
					<view class="tubiao">
						<image class="img" :src="img('addon/business_agent/fenxiao/fxtd.png')"></image>
					</view>
					<view class="tt">
				...
				<view class="ico">
					<view class="tubiao">
						<image class="img" :src="img('addon/business_agent/fenxiao/yidd.png')"></image>
					</view>
					<view class="tt">
				...
				<view class="ico">
					<view class="tubiao">
						<image class="img" :src="img('addon/business_agent/fenxiao/erdd.png')"></image>
					</view>
					<view class="tt">
	...
	<share-poster ref="sharePosterRef" posterType="business_agent_poster" :posterId="poster_id"
		:posterParam="posterParam" :copyUrlParam="copyUrlParam" />
	<tabbar addon="business_agent" />
...
<script setup lang="ts">
	...
	import { getOrderInfo, getOrderStatus } from '@/addon/business_agent/api/order';
	import { getMemberCommission } from '@/app/api/member';
	import useMemberStore from '@/stores/member'
	import { onLoad, onPageScroll, onReachBottom } from '@dcloudio/uni-app';
	import { getUnit, getType, getDeliveryType } from '@/addon/business_agent/api/project'
	import {
		checkFenxiao, getFenxiaoInfo
	} from '@/addon/business_agent/api/fenxiao'
	...
	const shareEvent = () => {
		...
		if (!userInfo.value) {
			...
			if (pid && pid > 0) {
				useLogin().setLoginBack({ url: '/addon/business_agent/pages/project/list?mid=' + pid })
				return false
			} else {
				useLogin().setLoginBack({ url: '/addon/business_agent/pages/project/list' })
				return false
			}
		}
...
<style lang="scss" scoped>
	@import '@/addon/business_agent/utils/styles/common.scss';
