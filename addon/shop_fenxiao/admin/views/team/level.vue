                    ...
                    <el-form-item :label="t('teamLevel')">
                        <el-table :data="teamLevelData.level" size="large" ref="goodsListTableRef" @selection-change="handleSelectionChange">
                            ...
                            <el-table-column prop="team_rate" :label="t('bonus')" min-width="200">
                                <template #default="{ row }">
                                    <el-input v-model="row.team_rate" maxlength="6" show-word-limit @change="inputChange($event,'team_rate',row)" clearable class="!w-[180px]" @keyup="filterDigit($event)" :placeholder="t('setBonus')">
                            ...
                            <el-table-column prop="team_flat_rate" :label="t('bonus_flat')" min-width="300">
                                <template #default="{ row }">
                                    <el-input  v-model="row.team_flat_rate" maxlength="6" show-word-limit  @change="inputChange($event,'team_flat_rate',row)"  clearable class="!w-[180px]" @keyup="filterDigit($event)" :placeholder="t('setBonus')">
                                        <template #append>%</template>
                                    </el-input>
                                </template>
                            </el-table-column>
