 ...
 * 任务配置
 */
class CoreTaskConfigService extends BaseCoreService
{
    //系统配置文件
    ...
    public function setConfig(int $site_id, array $data)
    {
        ...
        $this->core_config_service->setConfig($site_id, 'SHOP_FENXIAO_TASK_CONFIG', $value);
        return true;
    }
    ...
    {
        $data = $this->core_config_service->getConfigValue($site_id, 'SHOP_FENXIAO_TASK_CONFIG');
        $value = [
            'is_open' => $data['is_open'] ?? '0',
        ];
        return $value;
