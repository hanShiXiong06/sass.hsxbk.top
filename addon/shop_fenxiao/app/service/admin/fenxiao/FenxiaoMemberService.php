...
class FenxiaoMemberService extends BaseAdminService
{
    ...
    public function getPage(array $where = [])
    {
        ...
        if (isset($where['search']) && $where['search'] != '') {
            $condition[] = ['member.member_no|member.username|member.nickname|member.mobile', 'like', '%' . $this->model->handelSpecialCharacter($where['search']) . '%'];
        }
        //是否是分销商
    ...
    public function getMemberListOfSelect(array $where = [])
    {
        ...
        if (!empty($where['search'])) {
            $condition[] = ['member.member_no|member.username|member.nickname|member.mobile', 'like', '%' . $where['search'] . '%'];
        }
        //不包括
