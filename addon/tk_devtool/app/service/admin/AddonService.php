...
class AddonService extends BaseAdminService
{
    ...
    public function build(string $addon)
    {
        ...
        $zip_file = public_path().'/dowloadaddon/'.$addon.'/' . $addon.' saas '.$addonArr['version'] . '.zip';
        ...
        return (new CommonService())->getUrl().'/dowloadaddon/'.$addon.'/' . $addon.' saas '.$addonArr['version'] . '.zip';
    }
    /**
     * 下载
     * @param string $addon
    ...
    public function uploadAddon($data)
    {
        ...
        if (empty($addoning)) {
            if (is_dir($addonPath)) {
                $this->recursiveRemoveDirectory($addonPath);
            }
