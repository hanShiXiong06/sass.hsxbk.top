								...
								<el-table :data="item.order_goods" size="large" :show-header="false" :span-method="arraySpanMethod" ref="multipleTable" @select="handleSelectChange">
									...
									<el-table-column align="right" min-width="120">
										<template #default>
                                            ...
                                            <el-button type="primary" v-if="(item.status == 1 || item.status == 2) && item.delivery_type != 'virtual' && item.activity_type != 'giftcard'" link @click="orderEditAddressFn(item)">{{ t('editAddress') }}</el-button>
