                ...
                <div class="table-body h-[350px] overflow-y-auto">
                    <div v-for="(row,rowIndex) in goodsTable.data" :key="rowIndex" class="flex flex-col">
                        ...
                        <div class="flex items-center border-solid border-[#e5e7eb] py-[8px] border-b-[1px]">
                            ...
                            <div v-if="prop.mode == 'sku'" class="w-[3%] cursor-pointer text-center !text-[10px]" @click="secondLevelArrowChange(row)" :class="{'iconfont iconxiangyoujiantou': row.skuList.length, 'arrow-show': row.isShow}"></div>
                            <div class="w-[7%]">
...
const detectionAllSelect = ()=> {
    ...
    }else if(selectNum == goodsTable.data.length){
        isStairIndeterminate.value = false;
    ...
    loadGoodsList(1, (verify_ids: any) => {
        ...
        if (goodsIds.value) {
            ...
            goodsIds.value.forEach((item: any) => {
                if (!selectGoods[replacePrefix + item]) {
                    selectGoods[replacePrefix + item] = {};
                }
            })
        }
    })
