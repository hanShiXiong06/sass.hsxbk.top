	...
	<view class="bg-[var(--page-bg-color)] min-h-[100vh]" :style="themeColor()">
		<view class="fixed left-0 right-0 top-0 product-warp bg-[#fff]">
			...
			<view class="h-[88rpx] px-[30rpx]">
				<view class=" flex items-center text-[26rpx] text-[var(--text-color-light6)] h-[88rpx]">
					<text class="mr-[120rpx]" :class="{ '!text-[var(--primary-color)] font-500': searchType == 'total_order_num' }" @click="searchTypeFn('total_order_num')">综合排序</text>
					<view class="flex items-center mr-[120rpx]" @click="searchTypeFn('total_exchange_num')">
