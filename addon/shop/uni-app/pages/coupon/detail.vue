<template>
	<view :style="themeColor()">
		...
		class="overflow-hidden min-h-screen bg-style relative" :style="{ 'background': 'url(' + img('addon/shop/coupon/coupon_bg.png') + ') no-repeat' }">
			<!-- #ifdef MP -->
			...
			<view class="relative mt-[236rpx] w-[100%] h-[932rpx]" :style="{ 'background': 'url(' + img('addon/shop/coupon/coupon_bg_02.png') + ') center / contain no-repeat' }">
				<view v-if="detail.limit_count"
					...
					class="text-[32rpx] leading-[56rpx] -top-[2rpx] left-[50%] px-[17rpx] box-border justify-center absolute min-w-[196rpx] h-[56rpx] flex items-center text-[#fff]">
				...
				<view class="text-[28rpx] text-[#E22D17] mt-[60rpx] text-center">
					<text v-if="detail.valid_type == 1">领取之日起<text>{{ detail.length}}</text>天内有效</text>
					<text v-else> 有效期至<text>{{ detail.valid_end_time ? detail.valid_end_time.slice(0, 10) : '' }}</text></text>
				</view>
				<view class="flex justify-center items-center mt-[24rpx]">
					<text v-if="detail.btnType === 'collected'" class="!leading-[98rpx] text-center text-[#999] text-[50rpx] min-w-[240rpx] h-[100rpx]"
						:style="{ 'background': 'url(' + img('addon/shop/coupon/coupon_btn_02.png') + ')  center / contain no-repeat' }"
						>已领完</text>
					<text v-if="detail.btnType === 'collecting'" class="!leading-[98rpx] text-center text-[#E22D17] text-[50rpx] min-w-[240rpx] h-[100rpx]"
						:style="{ 'background': 'url(' + img('addon/shop/coupon/coupon_btn.png') + ') center / contain no-repeat' }"
						 @click="collecting(detail.id)">领取</text>
					<text v-if="detail.btnType === 'using'" class="!leading-[98rpx] text-center text-[#E22D17] text-[50rpx] min-w-[240rpx] h-[100rpx]"
						:style="{ 'background': 'url(' + img('addon/shop/coupon/coupon_btn.png') + ') center / contain no-repeat' }"
