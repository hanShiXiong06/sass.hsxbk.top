            ...
            <view v-if="detail" class="bg-[#f7f7f7] min-h-screen overflow-hidden">
                ...
                <view class="mt-[20rpx]">
                    <view class="bg-[#fff] mx-[30rpx] p-[30rpx] rounded-[10rpx]">
                        <view class="order-goods-item  py-[20rpx]"
                            v-for="(goodsItem, goodsIndex) in detail.item" :key="goodsIndex">
                            <view class="flex  justify-between items-center" @click="toLink(goodsItem)">
                                ...
                                <view class="ml-[20rpx] flex-1">
                                    ...
                                    <view class="text-[24rpx] mt-[20rpx] flex">
                                        <text class="text-[var(--primary-color)] rounded-[6rpx] py-[6rpx] bg-[var(--label-bg-color)] px-[10rpx]">{{ goodsItem.item_type_name }}</text>
                                    </view>
                            ...
                            <view class="flex justify-end  self-end w-[100%] mt-[10rpx]">
                                <view v-if="goodsItem.refund_no && goodsItem.refund_status != '' && goodsItem.refund_status != 'cancel'"
                                    ...
                                    @click="redirect({ url: '/addon/o2o/pages/refund/detail', param: { refund_no : goodsItem.refund_no } })">{{ t('viewRefund') }}</view> 
                                <view v-if="detail.order_status_info.is_refund && goodsItem.is_enable_refund &&['','cancel'].indexOf(goodsItem.refund_status) != -1"
                                    class="text-[26rpx] leading-[56rpx] px-[30rpx] border-[3rpx] border-solid border-[#999] rounded-full ml-[10rpx]"
                                    @click="refundApplyFn(goodsItem.order_item_id)">{{ t('applyRefund') }}</view>
                                <block v-if="goodsItem.item_type == 'custom'">
                                    <view v-if="!goodsItem.pay_time"  class=" text-[26rpx] leading-[56rpx] px-[30rpx] border-[3rpx] border-solid border-[#999] rounded-full ml-[10rpx]"
                                    @click="orderBtnFn('item_pay',goodsItem)">{{ t('goToPay') }}</view>
                                    <view  class=" text-[26rpx] leading-[56rpx] px-[30rpx] border-[3rpx] border-solid border-[#999] rounded-full ml-[10rpx]"
                                    @click="showServiceFn(goodsItem)">{{ t('show') }}</view>
                                </block>
                            </view>
                        </view>
                    </view>
                    ...
                    <view class="bg-[#fff] mx-[30rpx] p-[30rpx] mt-[30rpx] rounded-[10rpx]">
                        ...
                        <view 
                            class="flex justify-between text-[28rpx] border-top-[2rpx] border-[solid] border-[#f1f1f1] mt-[30rpx] box-border">
                            <view class="shrink-0 mr-[18rpx]">{{ t('remark') }}</view>
                            <view class="max-w-[510rpx] leading-[38rpx] line-feed">{{ detail.member_message ? detail.member_message :'无' }}</view>
                        </view>
                ...
                <view
                    ...
                    <view class="flex justify-end mr-[30rpx]" v-if="detail.order_status_info">
                        <view v-for="(btnItem,btnIndex) in detail.order_status_info.member_action" :key="btnIndex" class="inline-block text-[26rpx] leading-[56rpx] px-[30rpx] border-[3rpx] border-solid border-[#999] rounded-full ml-[10rpx]"
                            @click="orderBtnFn(btnItem.key)">{{btnItem.name }}</view>
                    </view>
                </view>
            </view>
...
const showServiceFn = (data) =>{
	redirect({ url: '/addon/o2o/pages/master/task/show', param: { order_id: data.order_id, order_item_id: data.order_item_id, item_name: data.item_name, price: data.item_money, item_images: data.item_images } })
}
</script>
