<template>
  ...
  >
    <div
      ...
      class="h-[60vh]"
      v-loading="loading"
      :element-loading-text="正在执行检测编译环境"
    >
      <div
        class="h-[60vh] flex flex-col"
        v-if="cloudBuildCheck && !cloudBuildTask"
      >
        <el-scrollbar>
          <div class="bg-[#fff] my-3" v-if="cloudBuildCheck.dir">
            <p class="pt-[20px] pl-[20px]">
    ...
    <div v-if="active == 'complete'">
      <div class="h-[60vh] flex flex-col">
        <div class="flex-1 h-0">
          <el-result
            icon="success"
            :title="t('cloudbuild.cloudbuildSuccess')"
          ></el-result>
        </div>
      </div>
    </div>
...
const showElNotification = () => {
  notificationEl = ElNotification.success({
    ...
    message: h("div", {}, [
      ...
      h(
        "span",
        { class: "text-primary cursor-pointer", onClick: elNotificationClick },
        [t("cloudbuild.clickView")]
      ),
    ]),
    duration: 0,
    showClose: false,
...
const dialogClose = (done: () => {}) => {
  if (active.value == "build" && cloudBuildTask.value) {
    ElMessageBox.confirm(t("cloudbuild.showDialogCloseTips"), t("warning"), {
